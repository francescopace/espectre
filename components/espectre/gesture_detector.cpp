...
const char* GestureDetector::predict(const float* features, float* out_confidence, float* out_margin) {
    ...
    float logits[C];
    for (int j = 0; j < C; j++) {
        float acc = GESTURE_LOGREG_INTERCEPTS[j];
        const int row_base = j * GESTURE_NUM_FEATURES;
        for (int i = 0; i < GESTURE_NUM_FEATURES; i++) {
            const float x = (features[i] - GESTURE_FEATURE_MEAN[i]) / GESTURE_FEATURE_SCALE[i];
            acc += GESTURE_LOGREG_COEFS[row_base + i] * x;
        }
        logits[j] = acc;
    }
