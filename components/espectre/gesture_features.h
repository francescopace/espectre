 ...
 *   0. event_duration       - Log-compressed duration (bounded)
 ...
 *   4. pre_post_energy_ratio - Energy ratio first half / second half
 *   5. n_local_peaks        - Normalized count of local maxima (count/10)
 ...
 *   9. phase_diff_var       - Mean phase differential variance across event
 *  10. phase_entropy        - Mean phase Shannon entropy across event
 *  11. phase_circular_variance - Mean circular variance across event
 *
...
constexpr uint8_t GESTURE_NUM_FEATURES = 12;
 ...
 * @param features_out   Output array for GESTURE_NUM_FEATURES values
 */
inline void extract_gesture_features(const float* turb_buf,
                                     ...
                                     uint8_t n_phases,
                                     float* features_out) {
    for (uint8_t i = 0; i < GESTURE_NUM_FEATURES; i++) features_out[i] = 0.0f;
    ...
    features_out[9]  = gesture_phase_diff_var(phases_buf, n_phases, n_packets);
    features_out[10] = gesture_phase_entropy(phases_buf, n_phases, n_packets);
    features_out[11] = gesture_phase_circular_variance(phases_buf, n_phases, n_packets);
}
