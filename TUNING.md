...
> **Note on Subcarrier Selection**: ESPectre automatically selects optimal subcarriers using P95 Band Selection at first boot. No manual configuration needed.
...
**What it does:** Automatically calculates the optimal detection threshold based on baseline noise characteristics. This ensures zero false positives while maintaining high recall.

**Status:** Always enabled (automatic)

**How it works:**
1. During calibration, collects ~700 CSI packets during baseline (room quiet)
2. Calculates P95 (95th percentile) of moving variance for the selected band
3. Sets threshold = P95 × 1.4 (safety margin for zero FP)
4. Result: threshold adapts to each environment's noise level

**Formula:**
```
adaptive_threshold = P95(baseline_moving_variance) × 1.4
```

**Benefits:**
- **Zero false positives** guaranteed (threshold above 99%+ of baseline noise)
