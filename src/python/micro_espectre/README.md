...
`TRAFFIC_GENERATOR_MODE` accepts `ping`, `dns`, or `dns_tcp`; the committed configuration selects `ping` for every supported chip, and `config_local.py` can select another mode for a deployment. Set `TRAFFIC_GENERATOR_TARGET_IP = "192.168.1.53"` in `config_local.py` to override the destination for all three modes; the default empty string uses the current Wi-Fi gateway. The destination must be a unicast IPv4 address, using the same address restrictions as the C++ frontends in [SETUP.md](../../../docs/SETUP.md#traffic-generation). Both DNS modes require a resolver on port 53 at that destination. Setting `TRAFFIC_GENERATOR_ENABLED = False` requires an external CSI traffic source. Micro-ESPectre has no runtime traffic mutation, external UDP marker listener, or multicast join. `CSI_CAPTURE_MAX_DATA_LEN` selects the fixed native ring-record stride: 256 supports the doubled HT20 layout, while 128 is suitable only when every captured frame uses the canonical payload because larger frames are truncated.
