...
Wi-Fi setup and recovery use the shared `CONFIG_ESPECTRE_WIFI_TX_RATE_MBPS` policy through the `espectre_native_wifi` firmware module. Before connecting, setup registers an association handler that applies the policy to each AP before IPv4 is available, including transparent reassociation. Setup and recovery check the result again before enabling CSI. The default is OFDM 6 Mbps, with Auto (`0`), 12, and 24 Mbps build-time overrides. The station rate remains independent of generator start and stop, including when traffic generation is disabled. Driver errors propagate before capture starts. [CSI.md](../../../docs/CSI.md#internal-generators) records the shared station-rate compatibility limits. Deploying this application requires rebuilding and flashing firmware with the native Wi-Fi module.
