 ...
 * 
 * MVS Formula: threshold = percentile(cal_values) x factor
 * 
 * Modes:
 * - "auto": P100 x 1.3 (default, lower false positives on no-gain-lock captures)
 * - "min": P100 x 1.0 (maximum sensitivity, may have FP)
 * 
...
constexpr uint8_t DEFAULT_ADAPTIVE_PERCENTILE = 100;
...
enum class ThresholdMode {
  AUTO,    // P100 x 1.3 (default)
  MIN,     // P100 x 1.0 (maximum sensitivity)
 ...
 * 
 * MVS: threshold = percentile(cal_values) x factor
 * 
