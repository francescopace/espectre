...
void visit_runtime_diagnostics(const RuntimeConfig &config,
  ...
  std::snprintf(value, sizeof(value), "%.2f (%s)", snapshot.threshold, threshold_mode_name(config.threshold_mode));
  ...
  visitor("gain_locked", snapshot.gain_locked ? "true" : "false");
  ...
  std::snprintf(value, sizeof(value), "%.4f", snapshot.best_pxx);
