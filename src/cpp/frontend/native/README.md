...
BSSID changes are staged. Native suspends sensing, attempts the selected access point, commits it only after association and address acquisition, and rolls back to the last-known-good settings when the attempt fails or times out. The verified reconnect normally resets the CSI pipeline and starts a fresh calibration without rebooting the device. After a live rearm, the runtime waits for managed traffic and verifies that CSI callbacks resume; if traffic continues without a callback, it records a fault and restarts the device once into the already persisted configuration. Direct and MQTT clients reconnect after the device address becomes reachable again. If an optional BSSID is unavailable, the provisioning policy can retry the same SSID without the pin instead of permanently stranding the device.
