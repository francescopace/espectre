...
The maintained examples provision Wi-Fi through Improv Serial or the `ESPectre Fallback` captive portal and do not embed SSID, password, or BSSID in YAML. After the device is on the LAN, use the mesh Wi-Fi procedure in [`TUNING.md`](../../../../docs/TUNING.md#mesh-wi-fi-instability) if its access-point association is unstable. Direct `set_wifi_bssid` acknowledges the staged mutation with the current BSSID before changing the station association. When the requested BSSID is already active, the default behavior only persists the ESPectre pin; `force=true` performs the reassociation anyway. A new candidate is saved to non-volatile storage before ESPHome restarts its Wi-Fi state machine. ESPectre then verifies the selected association and IPv4 acquisition, commits the pin, and starts a fresh CSI session and calibration without rebooting the device. A failed update performs one restart with the previous pin and clears the pending candidate. The command does not rewrite the ESPHome YAML or saved SSID and password.
