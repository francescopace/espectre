...
- Apply and verify the optional target BSSID through Direct where supported. Send `force=true` so the setup exercises the Wi-Fi transition even when the device is already associated with that access point. Require the successful Direct acknowledgement, including its pre-apply `current_bssid`, before reconnecting; a timeout, reset, or dropped response is not acceptance evidence. Record whether Direct uptime evidence observed an unexpected reboot during the apply, but keep that observation informational and outside PASS or FAIL.
