...
The command writes a partial report when a case fails and returns success only when every selected case passes. It also preserves redacted raw logs, host-monotonic line timestamps, structured analysis, and a run manifest under `data/untracked/firmware_benchmarks/<run-id>/`; pass `--artifacts-dir <path>` to select an explicit run directory. Native, Micro-ESPectre, ESPHome, and Streamer collect cases pass when mean CSI occupancy stays at or above the 70% admitted-slot floor. Native, Micro-ESPectre, and ESPHome runtime monitoring wait for the first `IDLE`/`MOTION` heartbeat, then for five consecutive 1 Hz status lines, then measure the configured duration. The ESPHome cases use the native API to force and confirm the representative `internal` CSI traffic mode, `ping` traffic source, and requested detector before each scored window, so persisted Home Assistant selections cannot change the case under test. Native High Accuracy starts its stability wait after the MQTT detector switch, Micro-ESPectre Lightweight starts from a fresh production deploy, and ESPHome starts its stability wait after the native-API profile confirmation. Heap-decline scoring begins 10 seconds after the first heartbeat in the scored window. Micro-ESPectre debug telemetry records both pre-GC and post-GC heap, GC pause duration, and packet-processing timing; its heap guard uses the comparable post-GC samples rather than arbitrary collector phases. Device-clock regressions are recorded as uptime restarts and fail the runtime case. While uptime remains monotonic, cadence and expected-sample scoring use device timestamps and the observed median interval so normal scheduler drift does not accumulate into false missing-sample failures; after a reboot, the host monotonic clock keeps intervals positive. The parser recovers multiple ESP-IDF records concatenated after a truncated serial write, records the event as a serial framing anomaly, and still fails any real device-time gap over the cadence tolerance. Matter smoke still checks boot and commissioning only. Each report is a snapshot of the Git revision, hardware, environment, and run time recorded in its header; it does not certify later source revisions. Do not edit or reformat generated chip reports separately from a hardware benchmark run.

Expected runtime status counts tolerate one sample at the scored-window boundary. Recovered serial framing anomalies remain visible in the report but do not fail an otherwise continuous runtime window; any internal device-time gap over the cadence tolerance still fails the case.
