...
- Use Direct responses, diagnostics, and events for runtime configuration, validation, and metrics. Scored samples request the fixed `BENCHMARK_DIAGNOSTIC_FIELDS` selection directly; do not fetch the field catalog before a run. Handshake requests the Direct transport group, readiness requests admitted CSI rate, and SSE teardown requests only `direct_http.event_clients`. Use serial output only to detect fatal firmware errors, unexpected resets, or an unexpectedly terminated monitor.
