...
Native, ESPHome, and Matter reuse one flashed canonical Lightweight image and select both scored detectors through Direct. BSSID verification waits for any reported staged transition to complete and rejects rollback or recovery states, even when the current association already matches the requested access point.
