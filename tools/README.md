...
The benchmark always passes `--chip` to `./espectre` and delegates canonical config selection, build artifact resolution, serial selection, chip verification, full-data erasure during flash, provisioning, onboarding reads, Direct discovery filtering, and monitoring to the repository CLI. Delegated build, provisioning, Matter onboarding, and Micro Direct-ready events use final machine-readable JSON objects rather than human-output parsing. Omit `--port` to auto-select one unambiguous canonical console. When several equally suitable consoles remain, choose interactively before the CLI probes or resets any board. The selected flash candidate is then verified as the requested chip, including when only one candidate was available. The benchmark resolves that port once before starting the matrix and passes the same explicit port to every subsequent flash, provision, monitor, deploy, and run step. This excludes UART bridges for chips whose firmware uses USB CDC or USB Serial/JTAG, while retaining UART for the classic ESP32. Pass `--port` to require that exact compatible device. Explicit and auto-selected ports use the CLI's bounded USB re-enumeration wait before each operation. UART and USB Serial/JTAG flash operations delegate automatic loader entry to the flasher; only USB CDC requires a manually preserved download mode. Matter is omitted automatically for ESP32-S2 because the supported commissioning flow requires Bluetooth, and Micro-ESPectre is omitted because ESP32-S2 is not a maintained Micro target.
