...
The generator summary shows the configured rate per target and send errors since app startup. These values do not prove reception. On each device, **Traffic / s** means internal traffic sent in internal mode or external traffic received in external mode. **Occupancy** is valid CSI coverage, not the motion/occupancy binary sensor exposed by Matter.
