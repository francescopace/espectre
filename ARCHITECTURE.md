...
For ESPHome, `src/frontend/esphome` is the external-components search root and `espectre/` remains the component directory inside it.
...
For that reason, `src/frontend/esphome/espectre/` acts as both:

- the ESPHome frontend adapter, and
- the packaging entry point for the shared code now stored in `src/core/` and `src/runtime/`
...
- the ESPHome build uses `src/frontend/esphome`
...
4. [test/README.md](test/README.md) for validation strategy
5. [micro-espectre/ALGORITHMS.md](micro-espectre/ALGORITHMS.md) for algorithm details
