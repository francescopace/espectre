...
- **The production detectors share one runtime contract**: Classic uses vote-free weighted fusion of turbulence autocorrelation and channel frequency-coherence curve spread, while ML uses the promoted seven-feature `7 -> 24 -> 12 -> 1` phaseless model. Both publish movement on a `0.0–1.0` probability scale.
...
- **Classic detection now uses its final v3 two-feature model**: gain-invariant turbulence autocorrelation and channel frequency-coherence curve spread feed a fixed weighted logistic fusion. Startup calibration adapts the probability threshold to the current session and can recover once an unrepresentative startup period settles.
- **ML detection now uses the promoted compact seven-feature model** with 505 parameters. Training, Python replay, C++ replay, and firmware inference use the same exported features and arithmetic contract. Current performance and parity evidence is published in [performance/README.md](performance/README.md).
...
- **Motion activation now requires four consecutive evaluation hits by default**, corresponding to approximately one second at the default 100 packets per second cadence.
...
- **The ESPHome baseline is now `2026.6.0`**, and the host and ML workflows now require Python `3.14`.
