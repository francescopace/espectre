...
- **CSI target configuration is now `csi_target_pps`**. Replace `traffic_generator_rate`, `TRAFFIC_GENERATOR_RATE`, `RuntimeConfig::traffic_generator_rate`, and `CONFIG_ESPECTRE_TRAFFIC_GENERATOR_RATE` with `csi_target_pps` plus an explicit `csi_traffic_mode`. A rate of zero is invalid and no longer disables traffic; use `csi_traffic_mode: external` or `disabled` instead. Device internal pacing always uses that configured cadence; occupancy-adaptive send-rate trials and `traffic_generator_adaptive` / `RuntimeConfig::traffic_generator_adaptive` / `CONFIG_ESPECTRE_TRAFFIC_GENERATOR_ADAPTIVE` / MQTT `traffic_adaptive` were removed.
