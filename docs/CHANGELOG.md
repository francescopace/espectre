...
- **Published firmware now separates installation and update formats**: releases and snapshots publish 15 full-flash images for ESPHome, Matter, and native, plus 10 application-only OTA payloads for ESPHome and native. GitHub Pages stages only the full-flash images, while native firmware resolves its per-chip OTA manifest from GitHub Releases by default. Streamer remains source-built because its Wi-Fi credentials are supplied at build time.
