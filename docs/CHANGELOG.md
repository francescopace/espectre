...
- **Raw CSI collection uses one HTTP v2 path across Native, ESPHome, and Matter**. The host collector discovers raw-capable Direct endpoints, persistently selects external traffic, imports the standalone external UDP generator, and identifies its traffic with the exact one-byte marker `b'.'` (`0x2E`). It receives transport-neutral CSI V8 records and preserves PHY metadata in saved captures. HTTP applies no pacing or temporal decimation; a fixed 16-record ring exposes raw drops explicitly. Post-capture validation reuses the dataset validator, warns below 85% temporal occupancy, fails below 70%, and caps quality scores by occupancy.
