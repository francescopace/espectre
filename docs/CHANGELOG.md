...
- **The production detection profiles share one runtime contract**: Lightweight uses vote-free weighted fusion of turbulence autocorrelation and aggregated turbulence IQR, while High Accuracy uses the promoted seven-feature `7 -> 24 -> 12 -> 1` phaseless ML model. Both publish movement on a `0.0–1.0` probability scale.
...
- **ESPHome intensity and on-demand CSI diagnostics**. The new intensity sensor stays meaningful across automatic Lightweight calibration, while the diagnostics surface reports traffic, CSI callback, accepted, admitted, filtered, missing-slot, excess, stale, and out-of-order rates plus occupancy, Wi-Fi channel, and RSSI only when requested.
- **Native on-demand MQTT diagnostics** for traffic, CSI accepted and admitted rates, temporal drop counters, occupancy, Wi-Fi channel, and RSSI through the `stats` command. The extra CSI fields are additive on protocol `1.0`.
...
- **Detector windows now use fixed temporal CSI admission**: `csi_target_pps` and `segmentation_window_size_ms` define one stable slot grid, the candidate nearest each slot center is retained with a target-derived half-slot minimum spacing, a window-sized gap invalidates detector history immediately, missing slots remain invalid, and 80% occupancy is required for readiness. Arrival jitter no longer reconstructs detectors or restarts Lightweight calibration; Micro-ESPectre, collector sensing, replay, training, Python validation, and C++ integration replay reuse their production sampler, while Streamer preserves raw collector-paced transport.
...
- **Device CSI traffic generation uses fixed cadence by default** across ESPHome, Native, Matter, and Micro-ESPectre, so unrelated application CSI cannot make the managed sensing source slow down. Adaptive device pacing remains opt-in, while host collection retains adaptive pacing and `--fixed` remains available for constant-rate experiments.
...
- **Detector identifiers and SDK class names changed without compatibility aliases**. Replace `classic` with `lightweight` and `ml` with `high_accuracy` in ESPHome YAML, Micro-ESPectre configuration, CLI arguments, BLE and MQTT commands, persisted selections, and protocol consumers. C++ SDK users must replace `DetectionAlgorithm::CLASSIC`, `DetectionAlgorithm::ML`, `ClassicDetector`, and `MLDetector` with `DetectionAlgorithm::LIGHTWEIGHT`, `DetectionAlgorithm::HIGH_ACCURACY`, `LightweightDetector`, and `HighAccuracyDetector`, respectively.
...
- **Direct `LightweightDetector` integrations must use the aggregated-IQR surface**. The C++ coherence-lag constructor argument and `get_chan_freq_coh_curve_std()` accessor were removed; use the three-argument constructor ending in `autocorr_lag` and `get_turb_iqr_over_mean_aggr()`.
...
- **CSI target configuration is now `csi_target_pps`**. Replace `traffic_generator_rate`, `TRAFFIC_GENERATOR_RATE`, `RuntimeConfig::traffic_generator_rate`, and `CONFIG_ESPECTRE_TRAFFIC_GENERATOR_RATE` with `csi_target_pps` plus an explicit `csi_traffic_mode`. A rate of zero is invalid and no longer disables traffic; use `csi_traffic_mode: external` or `disabled` instead. Device internal pacing now defaults to fixed cadence (`traffic_generator_adaptive: false`); adaptive device pacing is opt-in.
