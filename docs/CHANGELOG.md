...
- **Native BLE, Native MQTT, Micro MQTT, and ESPHome now share one sensing-control family** for threshold, motion-hit debounce, recalibration, CSI traffic ownership, and traffic generator selection. Native and ESPHome persist the supported runtime settings, while Micro keeps them session-only and explicitly rejects CSI pacing mode.
