...
- **Home Assistant MQTT Discovery for Native and Micro-ESPectre**. Discovery is enabled by default on every supported chip. The adapter includes writable Threshold and Motion Hits numbers, a Calibrate switch, CSI traffic-control selects, the ESPHome CSI diagnostic sensors, and Refresh Diagnostics alongside Motion Detected and Movement Score.
...
- **Every Micro-ESPectre target now uses one performance-oriented project firmware profile** instead of generic official images. ESP32, ESP32-C3, ESP32-C5, ESP32-C6, and ESP32-S3 retain filesystem-deployed `.mpy -O3` application updates, require native MQTT and STA-bound ICMP or DNS traffic modules without Python transport fallbacks, enable ESP-IDF-managed MQTT reconnects and a retained offline Last Will, and share compiler performance optimization, a 1 kHz FreeRTOS tick, balanced Wi-Fi queues, disabled power management, and Wi-Fi, PHY, and lwIP IRAM placement. Classic ESP32 alone uses smaller Wi-Fi queues and omits lwIP IRAM placement to preserve heap; RX AMPDU remains disabled for individual HT20 CSI delivery on every target.
- **Micro-ESPectre now shares the 100 pps target, 70% valid-slot floor, default MQTT availability, and Home Assistant Discovery behavior of the ESP-IDF sensing frontends**. Hardware validation on the original ESP32 sustained the fixed target with native traffic and MQTT.
- **Micro-ESPectre device identity now matches the ESP-IDF frontends**, deriving the protocol and MQTT client identifier from the station MAC through the shared SHA-256 pseudonym instead of accepting a configured `MQTT_CLIENT_ID`.
- **Configure and Monitor disconnected states now match the Game and Theremin hierarchy**: first-setup, recovery, and already-configured context moved into the page subtitles, redundant card kickers and styles were removed, and user-facing setup copy consistently says Bluetooth while BLE remains the compact technical label.
...
- **The published ESP-IDF SDK baseline is now 5.5**, matching the complete frontend and chip matrix, including production ESP32-C5 support. Reproducible repository and CI builds remain pinned to ESP-IDF 5.5.5.
...
- **Lightweight settled-level threshold recovery now publishes `on_threshold_changed`**, so Home Assistant, ESPHome, and the website Monitor control show the live adapted threshold instead of remaining on the post-calibration snapshot. Monitor also mirrors the live telemetry threshold into the input unless that field is being edited.
...
- **The website BLE and MQTT transports now use standalone protocol clients**. BLE serializes GATT writes, cancels stale connection work, validates firmware byte limits, and emits only complete sysinfo frames. MQTT centralizes topic discovery, protocol-version and device-identity validation, command correlation, timeouts, acknowledgements, Home Assistant mirrors, and protocol errors. Monitor consumes that shared MQTT surface instead of maintaining a parallel parser.
- **The website BLE and MQTT protocol clients now follow the repository-wide GPLv3 and commercial licensing policy**, removing the separate Apache-2.0 exception and its dedicated license artifact.
- **MQTT `info` now includes `evaluation_interval_ms` and `publish_interval_ms`** alongside `csi_target_pps`, so clients can follow the device cadence instead of assuming 250 ms or 1 Hz. Native and Micro-ESPectre publish the live runtime values; omit them when unset. Website Monitor diagnostics poll at `publish_interval_ms`.
- **Canonical MQTT `info` no longer publishes the device's local IP address or station MAC.** Native and Micro-ESPectre retain the active Wi-Fi channel for diagnostics, while the persistent protocol `device_id`, user-provided label, and other operational fields remain part of the message and require appropriate broker access and retention controls. Native, Matter, and Streamer now derive the ID once from the first 64 bits of a domain-separated SHA-256 over the station MAC, cache it, and render it as 16 lowercase hexadecimal characters without `0x`; this changes their MQTT topics, Home Assistant entity IDs, BLE identity, Streamer discovery identity, and future collection metadata after a firmware upgrade. Existing Streamer packet and `.npz` fields remain `uint64`, and previously collected files are unchanged.
- **The Game movement bar now has a draggable hit-line marker.** Opening Game copies the live 0–1 device threshold into a browser-only hit line and keeps that value until the page is opened again, so live calibration does not move the marker. Dragging retunes only false-start and hit detection, and does not send `set_threshold` to the device. Monitor still writes the device threshold.
...
- **Disconnected-state alternative links now keep the standard accent treatment** before and after activation, with underlining reserved for hover instead of appearing permanently.
- **The homepage release badge now loads the published `release` firmware manifest**, matching the release workflow and Flash channel instead of requesting the removed `stable` path.
- **Matter’s Flash next-step actions now use consistent links**, and onboarding codes read over USB open in an accessible modal instead of expanding the Install panel. The USB action preserves disabled and in-progress handling.
- **ESP-IDF callback and worker lifetimes now preserve single-task runtime ownership.** CSI frames, Wi-Fi events, MQTT messages, BLE controls, and OTA progress are handed to bounded loop-owned queues; detector replacement and calibration no longer race the Wi-Fi callback; Streamer stops and joins its direct TX worker before destruction; CSI callback registration rolls back on partial startup failure; and shared turbulence, stream serialization, MAC, counter, and station-socket helpers remove parallel implementations.
