...
- **MVS calibration is now simpler and faster**: the active runtime path no longer depends on NBVI calibration buffers and startup calibration is now used only to tune the adaptive threshold, with `auto` using the current `max x 1.3` startup baseline.
...
- **Dataset and sensing defaults were normalized across the project**: room-state labels were simplified, empty-room validation became part of the standard workflow, the active runtime path now uses one fixed shared subcarrier set, and MVS startup calibration was reduced to adaptive-threshold tuning only, with `max x 1.3` as the default startup threshold mode.
- **Default runtime subcarriers were moved away from the DC bin**: the shared fixed 12-subcarrier set is now `[14, 17, 20, 23, 26, 29, 35, 38, 41, 44, 47, 50]`, improving current MVS real-data validation while keeping one cross-chip default band.
...
- **Host-side analysis tooling was modularized internally**: shared helpers were split into `tools/lib/` so end-user entrypoints stay at the top level, with dataset metadata resolution, CSI I/O, plotting helpers, path helpers, and paired MVS sweep logic now living in focused internal modules instead of the old monolithic helper files.
- **The tooling support detector moved from MVS to l1_delta**: the legacy `optimal_threshold_gridsearch` metadata was retired, dataset pair validation and the detection-methods comparison now replay the production `L1DeltaDetector` startup calibration directly on the selected quiet capture, ML sample weighting uses l1_delta-guided modes (`l1_gridsearch`, `l1_hard_negative`), and MVS-specific research tools keep self-calibrating their thresholds instead of reading metadata.
