...
- **ESPectre is now an embeddable C++ SDK for hardware vendors and product builders**. Take `core` when you already own CSI capture and only need the detectors. Take the `runtime` when you want sensing, calibration, and events; ESP-IDF is the current backend, and other runtimes can implement the same contracts later. Take a dedicated `frontend` when you want a finished product surface. The published frontends are ESPHome, Native, and Matter.
- **Native is a new standalone frontend**. It is BLE-free ESP-IDF firmware. Sensing uses HTTP. MQTT and Home Assistant MQTT Discovery are optional. OTA uses HTTPS. Provisioning uses standard Improv Serial.
- **Matter is a new occupancy sensor firmware frontend** that publishes the standard occupancy sensor type. Each device generates its own onboarding data, and its QR code remains available from serial output, the web flasher, or `./espectre matter qr`. Sensing starts only after commissioning. Direct stores a BSSID preference for the commissioned SSID without changing Matter-owned credentials. The hardware benchmark commissions the device with a CHIP Tool build that matches the firmware, scores both detectors, and does not retain onboarding data or fabric secrets. Current images use development VID/PID and example device attestation credentials, do not support Matter OTA, and have limited controller validation.
- **Micro-ESPectre is a small, read-only sensing frontend**. The deployed application uses Lightweight Detector, the shared managed traffic generator with deployment-time `ping`, `dns`, or `dns_tcp` selection, five HTTP queries plus session-only recalibration, one telemetry SSE stream, mDNS discovery, and serial logging. Its profile enforces production Origin validation, a 10-second SSE heartbeat, a 512-byte request limit, a 4,096-byte event limit, one client, and one frame in flight. The High Accuracy ML model remains available in Python for host research and C++/Python validation but is not copied to the device.
- **ESPectre now supports two detectors: Lightweight and High Accuracy**. Lightweight is a low-cost two-feature weighted model with startup calibration. High Accuracy is an eight-feature phaseless neural model and does not need quiet-room threshold calibration. All production detector features are gain- and scale-invariant, so maintained firmware keeps AGC enabled instead of locking hardware gain.
...
- **ESP32-C5 integrations default to automatic 2.4/5 GHz selection and can be pinned to either band**. The runtime uses HT20 on 2.4 GHz and VHT20 on 5 GHz while excluding HE capture. The maintained examples select 2.4 GHz because detection quality on 5 GHz has not yet been characterized.
- **ESP32 and ESP32-S2 use LLTF20 capture automatically**. This admits legacy OFDM traffic while preserving the canonical centered 64-bin raw and detector views; other 2.4 GHz targets continue to use HT20.
- **One versioned protocol covers Direct HTTP and optional MQTT**. ESPectre frontends expose HTTP on port `62587`, advertise `_espectre._tcp.local.`, and enforce the methods declared by their capability profiles. Every frontend derives the same `ESPectre <chip> <short-id>` device name and keeps its optional user label empty until configured. ESPHome keeps this surface enabled by default and can disable HTTP, its discovery records, and browser bootstrap with `direct_api: false`. Supported detector, motion-hit, and traffic selections persist across reboot on the ESP-IDF frontends, while thresholds remain session-only. ESPHome republishes entity state after HTTP writes, and Matter uses HTTP for controls that its standard occupancy clusters do not expose.
- **Raw CSI collection uses HTTP across supported ESPectre frontends**. `./espectre collect` discovers raw-capable devices, selects external traffic, drives the shared UDP marker generator, and saves CSI V8 records with PHY and device provenance. HTTP does not pace or decimate records, and bounded firmware queues report drops explicitly. Post-capture validation warns below 85% temporal occupancy and fails below 70%.
...
- **The `./espectre` CLI covers the supported firmware and device workflows**. It builds, flashes, deploys, monitors, diagnoses, discovers devices, sends requests, collects CSI, and opens the interactive MQTT client from one command surface. Discovery uses the first-party `_espectre._tcp.local.` record rather than frontend-specific ESPHome or Matter services.
- **The [espectre.dev](https://espectre.dev) site provides browser-based flashing and device tools**. It provisions Native and ESPHome over standard Improv Serial, negotiates each device's HTTP capabilities, configures and monitors supported C++ frontends, manages Native MQTT and OTA settings, reads Matter onboarding data, and hosts the motion game and Wi-Fi Theremin. ESPHome images retain their fallback access point but no longer include BLE provisioning.
...
- **The embeddable C++ SDK has a documented public surface**. `espectre_sdk.h` exposes the runtime API, while `espectre_core_sdk.h` exposes lower-level detectors for custom CSI integrations. The embedding contract covers source compatibility, capabilities, ownership, threading, errors, optional components, and the absence of a stable binary ABI. Shared logging uses an optional frontend-owned sink, so the SDK does not require `esp_log` and remains silent when no backend is registered. Micro-ESPectre follows the same boundary: its core-only and traffic components remain logger-independent, while its native frontend registers the ESP-IDF adapter. Tagged SDK prereleases remain visible in the release channel but are marked explicitly as evaluation builds rather than production candidates.
- **Diagnostics report CSI callback-queue occupancy and overflow**. ESPHome consumes Wi-Fi state events instead of polling the station interface, and Direct consumers use a cached SSE client count. Runtime setup restores a station that connected before handler registration and preserves staged frontend configuration across live runtime callbacks. Invalid Kconfig-derived values fall back to documented defaults. Setup reports bounded-storage allocation failures without throwing, and the generated API uses consumer-relative include paths. Raw CSI callbacks have an explicit bounded, non-blocking, allocation-free contract, and OTA quiescing stops an active raw session safely. Bundled SDK guides link to the commit or release tag used to build each package.
...
- **Micro-ESPectre now uses `espectre-<device_id>.local`**. Update manually saved endpoints or rediscover the device; the former `espectre-micro-<suffix>.local` hostname has no compatibility alias. Its `info` and mDNS TXT records now use the ESP-IDF application version and target chip.
- **The supported build and hardware baseline changed**. ESPHome requires at least `2026.7.0`, host and ML workflows require Python `3.14`, ESP-IDF integrations require ESP-IDF `>= 5.5`, and PlatformIO-backed builds are no longer supported. ESP32-S2 is supported by ESPHome and Native, but not Matter or Micro-ESPectre.
