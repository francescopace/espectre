...
- **MVS calibration is now simpler and faster**: the active runtime path no longer depends on NBVI calibration buffers and startup calibration is now used only to tune the adaptive threshold, with `auto` using the current `P100 x 1.3` startup baseline.
