...
- **Classic startup calibration is now motion-first, with an internal quiet-first fallback**: the active runtime path still calibrates only the adaptive threshold on the Classic primary metric, but it can now finish early from a validated `quiet -> motion -> quiet` startup pattern instead of always waiting for a fixed quiet-only window.
...
- **L1-Delta was promoted into the Classic runtime path**, in both the Micro-ESPectre runtime and the shared C++ core: the mean L1 displacement of the per-packet normalized amplitude profile (lag 10, window 100) is now the primary non-ML motion metric inside `ClassicDetector`. Selected by an offline benchmark of 9 candidate metrics over the repo datasets, it matches the historical moving-variance baseline aggregate quality with more uniform per-chip recall (S3: 95% vs 80% on the C++ validation datasets), a quiet level that varies <=1.3x across sessions (vs up to 14.5x for the moving-variance baseline), and a ~20% lower per-packet cost thanks to an allocation-free O(1) running-mean evaluation with no Hampel sorting. Startup calibration reuses the shared flow with a detector-specific `auto` factor (`threshold_metric x 1.1`). See `docs/ALGORITHMS.md` and `docs/EXPERIMENTS.md`.
- **Motion-first startup calibration for Classic thresholds**, in both the Micro-ESPectre runtime and the shared C++ runtime: startup now builds a stable quiet anchor, confirms a sustained motion chunk, waits for post-motion quiet, and derives the threshold inside the validated quiet/motion gap. If startup never confirms a trustworthy motion segment before the configured budget, the same shared calibrator falls back internally to the quiet-only ring statistic instead of exposing a second runtime mode. The startup budget remains `10 x window_size` by default, but it is now a maximum rather than a fixed wait, so clean startups may complete earlier while no-motion startups still converge safely. The startup variance floor is now frozen only from validated quiet chunks, so the Classic recovery vote no longer depends on pre-freeze samples collected blindly during startup.
