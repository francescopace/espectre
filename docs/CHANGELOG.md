...
- **One versioned protocol covers Direct HTTP and optional MQTT**. Native, ESPHome, and Matter expose Direct HTTP on port `62587`, advertise `_espectre._tcp.local.`, and enforce the methods declared by their capability profile. Supported detector, motion-hit, and traffic selections persist across reboot on the ESP-IDF frontends, while thresholds remain session-only. ESPHome republishes entity state after Direct writes, and Matter uses Direct for controls that its standard occupancy clusters do not expose.
- **Raw CSI collection uses HTTP v2 across Native, ESPHome, and Matter**. `./espectre collect` discovers raw-capable devices, selects external traffic, drives the shared UDP marker generator, and saves CSI V8 records with PHY and device provenance. HTTP does not pace or decimate records, and bounded firmware queues report drops explicitly. Post-capture validation warns below 85% temporal occupancy and fails below 70%.
...
- **Micro-ESPectre is a small, read-only sensing frontend**. The deployed application uses Lightweight Detector, the native ICMP generator, five Direct HTTP queries, one telemetry SSE stream, mDNS discovery, and serial logging. High Accuracy ml model remains available in Python for host research and C++/Python validation but is not copied to the device.
