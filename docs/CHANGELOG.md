...
- **Canonical MQTT `info` no longer publishes the device's local IP address or station MAC.** Native and Micro-ESPectre retain the active Wi-Fi channel for diagnostics, while the persistent protocol `device_id`, user-provided label, and other operational fields remain part of the message and require appropriate broker access and retention controls.
