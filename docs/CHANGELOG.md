...
- **Sensing runtimes feed Classic/ML only HT20 CSI**: shared `CsiPipeline` (ESPHome, native, Matter), Micro-ESPectre, and the streamer all use the same HT20-only sensing contract. Host NPZ loaders, dataset-quality sensing checks, and the C++ test NPZ loader use the same HT20 view (`keep_all_phy=True` to inspect mixed-PHY captures). The host cnpy helper now sizes NumPy unicode (`U*`) arrays correctly so PHY string fields load with the right stride.
