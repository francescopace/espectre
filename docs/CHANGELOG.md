...
- **The website now uses MQTT for live detection**. Configure remains Bluetooth provisioning for Wi-Fi, MQTT, identity, and OTA. Stop BLE and start detection copies the saved broker host, credentials, topic prefix, and device id, opens the MQTT Monitor, connects over WebSockets (port 9001 by default), and stops BLE so Home, Game, Theremin, and Monitor can share one live MQTT session. Configure and Monitor expose Start BLE to reopen Native setup over MQTT `set_ble` when the Bluetooth list is empty. The MQTT Monitor on-demand diagnostics panel now shows CSI admitted rate and occupancy percent from the existing `stats` payload.

### Fixed

- **MQTT command validation failures now echo `command_id`, the original command name, and accepted values or ranges**, so the interactive shell can show the reject and return to the prompt instead of waiting out the ACK timeout.
