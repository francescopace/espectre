...
- **Conditional ESPHome CSI diagnostics** under `debug_telemetry: true`: Home Assistant receives actual traffic TX, raw CSI callback, accepted CSI, and filtered CSI rates, plus the current Wi-Fi channel and RSSI, and the cumulative CSI channel-change count at a low-rate five-second cadence.
