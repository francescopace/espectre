...
## [3.0.0-rc1] - 2026-08-27 - Modular multi-frontend platform
...
- **Matter is a new production frontend**. It publishes the standard occupancy sensor type. Each device generates its own onboarding data, and its QR code remains available from serial output, the web flasher, or `./espectre matter qr`. Sensing starts only after commissioning. Direct stores a BSSID preference for the commissioned SSID without changing Matter-owned credentials. The hardware benchmark commissions the device with a CHIP Tool build that matches the firmware, scores both detectors, and does not retain onboarding data or fabric secrets. Controller validation remains limited.
- **Micro-ESPectre is a small, read-only sensing frontend**. The deployed application uses Lightweight Detector, the native ICMP generator, five HTTP queries plus session-only recalibration, one telemetry SSE stream, mDNS discovery, and serial logging. Its profile uses strict production Origins, a 10-second SSE heartbeat, a 512-byte request limit, a 4,096-byte event limit, one client, and one frame in flight. The High Accuracy ML model remains available in Python for host research and C++/Python validation but is not copied to the device.
