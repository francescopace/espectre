...
- Runtime and integration hardening covers the boundaries exposed by that SDK. Diagnostics report CSI callback-queue occupancy and overflow. ESPHome consumes Wi-Fi state events instead of polling the station interface, and Direct consumers use a cached SSE client count. Runtime setup restores a station that connected before handler registration, reports bounded-storage allocation failures without throwing, and publishes consumer-relative include paths in the generated API. Raw CSI callbacks have an explicit bounded, non-blocking, allocation-free contract, and OTA quiescing stops an active raw session safely.
