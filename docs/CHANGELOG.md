...
- **Peer-assisted local discovery now finds every advertised C++ frontend by default**. Native provides the validated shared IPv4 bootstrap responder, whose bounded browse returns Native, Streamer, ESPHome, and Matter endpoints with their advertised ports. The portal negotiates the selected device's exact Direct capabilities and hides unsupported configuration and runtime controls. The published Direct transport boundary supports optional deferred request completion without breaking synchronous SDK implementations, while manual IP, device-ID, remembered-device, Improv, QR, and CLI fallbacks remain available.
