...
- **Matter is a new occupancy sensor firmware frontend** that publishes the standard occupancy sensor type. Each device generates its own onboarding data, and its QR code remains available from serial output, Improv Serial, the web flasher, or `./espectre matter qr`. Sensing starts only after commissioning. Direct stores a BSSID preference for the commissioned SSID without changing Matter-owned credentials. The hardware benchmark commissions the device with a CHIP Tool build that matches the firmware, scores both detectors, and does not retain onboarding data or fabric secrets. Current images use development VID/PID and example device attestation credentials, do not support Matter OTA, and have limited controller validation.
...
For detector design, feature decisions, integration guidance, and validation results, see [ALGORITHMS.md](ALGORITHMS.md), [FEATURES.md](FEATURES.md), [SDK.md](SDK.md), and [performance/README.md](performance/README.md).
