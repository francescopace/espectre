...
- **Detector windows now use fixed temporal CSI admission**: `csi_target_pps` and `segmentation_window_size_ms` define one stable slot grid, at most one packet is admitted per slot, missing slots remain invalid, and 80% occupancy is required for readiness. Arrival jitter no longer reconstructs detectors or restarts Lightweight calibration; Micro-ESPectre, collector sensing, replay, training, Python validation, and C++ integration replay reuse their production sampler, while Streamer preserves raw collector-paced transport.
