...
- **Classic startup calibration is now simpler and faster**: the active runtime path no longer depends on NBVI calibration buffers and startup calibration is now used only to tune the adaptive threshold, with `classic` using the current `max x 1.1` startup baseline on its primary metric.
- **The ML detector was refreshed for better cross-device stability**: the promoted model now uses a mixed turbulence and L1-delta feature set ("Core-6") that cuts long-quiet false positives by roughly 70% versus the previous relative-8 set, while staying lighter and gain-robust.
...
- **Matter frontend and release surface**:
  - standalone Matter firmware based on managed `esp_matter` with OTA support
...
- **L1-Delta was promoted into the Classic runtime path**, in both the Micro-ESPectre runtime and the shared C++ core: the mean L1 displacement of the per-packet normalized amplitude profile (lag 10, window 100) is now the primary non-ML motion metric inside `ClassicDetector`. Selected by an offline benchmark of 9 candidate metrics over the repo datasets, it matches the historical moving-variance baseline aggregate quality with more uniform per-chip recall (S3: 95% vs 80% on the C++ validation datasets), a quiet level that varies <=1.3x across sessions (vs up to 14.5x for the moving-variance baseline), and a ~20% lower per-packet cost thanks to an allocation-free O(1) running-mean evaluation with no Hampel sorting. Startup calibration reuses the shared flow with a detector-specific `auto` factor (`max x 1.1`). See `docs/ALGORITHMS.md` and `docs/EXPERIMENTS.md`.
- **Calibration consistency gate for Classic startup thresholds**, in both the Micro-ESPectre runtime and the shared C++ runtime: ready-state L1-delta calibration metrics are folded into a ring of 6 per-chunk maxima, and the startup window is accepted only when the ring is self-consistent (spread `<= 1.10x` the ring median, and median anchored to `<= 1.5x` the lowest chunk max). On rejection, calibration extends chunk by chunk (up to 2000 extra packets, with a `median x factor` fallback) and surfaces an extending status. On the paired datasets this keeps F1 at 94-95% even with a fully movement-contaminated calibration window, where the plain `max x 1.1` bootstrap fails closed (F1 down to 3%); clean startups keep the same threshold as before. Chunks discarded during extension whose peak stays within the floor-anchor band are treated as recurring quiet tail rather than motion, and their maximum is folded back into the threshold metric ("tail rescue"): this prevents the extension from locking a threshold below a session's own quiet-tail bumps, which produced a sustained 11.9% false-positive floor on a C6 long quiet run. Validated for the Classic primary metric only; the historical moving-variance baseline keeps the plain `max x 1.3` bootstrap in research tooling. See `docs/EXPERIMENTS.md`, "L1-Delta Contaminated-Calibration Gate And Extension Sweep" and "L1-Delta Quiet-Tail Rescue For Gate Extension".
- **Parallel multi-detector live collect**: `./espectre collect --detector` now accepts a comma-separated list (`classic,ml`) and runs the detectors side by side, with independent per-detector startup calibration and one live status line per device and detector for direct A/B comparison.
...
- **Dataset and sensing defaults were normalized across the project**: room-state labels were simplified, empty-room validation became part of the standard workflow, the active runtime path now uses one fixed shared subcarrier set, and Classic startup calibration was reduced to adaptive-threshold tuning only, with `max x 1.1` as the default startup threshold mode for the primary metric.
...
- **The production ML feature set is now the mixed "Core-6" set** (`turb_mad_over_mean`, `turb_skewness`, `turb_autocorr`, `l1_delta`, `l1_delta_std`, `l1_delta_waveform_length`), replacing the relative-8 turbulence set. Adding three L1-delta profile-displacement features cuts long-quiet false positives by ~70% (median 552 vs 1846) and raises grouped-CV OOF F1 to 83.1% (from 79.8%) while using fewer parameters (769 vs 833) and the same inference cost. The C++ feature extractor is now driven by an exported `ML_FEATURE_IDS` array instead of the model input count, and `MLDetector` maintains an incremental profile/delta ring (capacity `window_size - lag`) that exactly reproduces the Python L1-delta series. See `docs/EXPERIMENTS.md`, "Core-6 Mixed Turbulence + L1-Delta ML Feature Set".
...
- If baseline > 0.25: attenuate with `scale = 0.25 / static_presence_variance`
