...
- **The production ML feature set is now Coherence-6**: Core-6 with its two energy-based members (`turb_skewness`, `l1_delta_waveform_length`) swapped for shift/scale-invariant temporal-coherence statistics (`turb_zcr`, `l1_delta_autocorr`) that stay separable at low RSSI and collapse seed-to-seed false-positive variance.
...
- **Temporal-coherence candidate ML features and a `--features` trainer flag**: shift/scale-invariant candidates (`turb_zcr`, `l1_delta_autocorr`, `l1_delta_cv`) are selectable for training experiments without touching the production Core-6 set; `turb_zcr` and `l1_delta_autocorr` ship with C++ extractor ids and Python-parity tests, and exporting flows reject features without a C++ extractor. On real weak-link pairs the absolute L1 features lose or invert their motion separation, while the coherence swap candidate holds the reserved-replay max FP near `2.7%` across seeds versus `2.9`-`24.9%` for Core-6.
...
- **Matter now defaults back to Classic and is documented as a preview frontend**: published Matter firmware uses the conservative `classic` detector instead of `ml`, and the shared docs now present Matter as a published preview path while ESPHome remains the more mature production integration surface.
