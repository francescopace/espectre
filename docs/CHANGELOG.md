...
- **The Streamer frontend moves collection control to the PC**. The host collector discovers device IP addresses through mDNS, paces one or more devices, receives batched raw CSI over UDP, and preserves PHY metadata in saved captures.
...
- **One versioned protocol covers setup over BLE and operation over MQTT**. BLE provisions Wi-Fi, MQTT, identity, and OTA settings. MQTT handles discovery, telemetry, diagnostics, OTA, and live sensing commands.
...
- **The embeddable C++ SDK has a documented public surface**. `espectre_sdk.h` exposes the supported C++17 API, with version macros, configurable capability groups, CMake source lists, a vendored ESP-IDF component layout, Doxygen reference generation, and an embedding guide.
...
- **Direct ESPHome firmware URLs must use the new filename pattern**: `espectre-esphome-<channel-or-version>-<chip>.bin`.
