...
- **The production ML feature set is now Coherence-7**: Core-6 first swapped its two energy-based members (`turb_skewness`, `l1_delta_waveform_length`) for shift/scale-invariant temporal-coherence statistics (`turb_zcr`, `l1_delta_autocorr`), then gained the lag ratio Classic adopted (`l1_delta_lag_ratio`). Across the ten reserved replays that took effective alarms from `8` to `3` and the worst F1 from `92.92%` to `95.30%`. See [2026-07-27-add-the-lag-ratio-to-the-production-ml-feature-set.md](adr/2026-07-27-add-the-lag-ratio-to-the-production-ml-feature-set.md).
