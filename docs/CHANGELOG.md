...
- **Home Assistant MQTT Discovery for Native and Micro-ESPectre**. Discovery is enabled by default in published Native firmware and remains opt-in for Micro-ESPectre. The adapter now includes Intensity, writable Threshold and Motion Hits numbers, a Calibrate switch, and CSI traffic-control selects alongside Motion Detected and Movement Score.
- **ESPHome intensity and on-demand CSI diagnostics**. Intensity is the shared 0–100 movement-versus-threshold gauge used by ESPHome, Native MQTT Discovery, and Micro-ESPectre MQTT. The diagnostics surface reports traffic, CSI callback, accepted, admitted, filtered, missing-slot, excess, stale, and out-of-order rates plus occupancy, Wi-Fi channel, and RSSI only when requested.
...
- **Home Assistant Intensity now follows the detector evaluation cadence** (default `250 ms`) on ESPHome, Native MQTT Discovery, and Micro-ESPectre MQTT. Motion remains edge-published, and Movement Score stays on `publish_interval_ms`.
- **Home Assistant Intensity is now a 0–100 percent gauge with the decision threshold at 50%**. The previous 0–200 mapping, where 100% meant at threshold, is replaced by `min(100, movement / threshold × 50)` across ESPHome, Native MQTT, and Micro-ESPectre MQTT; 100% is twice the threshold.
- **Native MQTT, Micro MQTT, and ESPHome now share one sensing-control family** for threshold, motion-hit debounce, recalibration, CSI traffic ownership, and traffic generator selection. Native and ESPHome persist the supported runtime settings, while Micro keeps them session-only and explicitly rejects CSI pacing mode. Native BLE does not carry those sensing writes.
- **Native BLE is setup and recovery only**: the radio starts automatically when Wi-Fi or MQTT is unconfigured, pauses CSI while BLE is up, stays discoverable across nearby client disconnects, and stops only when `STOP_BLE` or MQTT `set_ble` with `ble=off` explicitly closes setup. MQTT `set_ble` or a three-second BOOT-button hold turns BLE back on for recovery. BLE exposes Wi-Fi, MQTT, identity, OTA, and read-only status. It does not publish live sensing or accept threshold, detector, traffic, or recalibrate commands.
...
- **The website now provides one Device console for setup and operation**. Nearby BLE provisions Wi-Fi, MQTT, and the device label; selecting Start sensing connects over MQTT WebSockets, waits for MQTT `set_ble off` to be accepted (and sends `STOP_BLE` when a nearby session is still open), then opens Live after valid device telemetry arrives. The device banner shows Start sensing while nearby setup is open and Edit connectivity otherwise. Edit connectivity publishes MQTT `set_ble on` and opens the browser Bluetooth picker as soon as the device accepts. Live sensing and runtime inputs share one status surface, changes apply directly on field change, and diagnostics remain collapsed below Live and refresh once per second while that section is open, with command-catalog capability gating and accepted/rejected command acknowledgements. The device banner subtitle labels chip, device ID, and firmware version. A silent update check reports Latest when the image is current, an Update action when a newer image is available, and the device or retrieval error when the OTA check fails. Connect with MQTT carries the browser WebSocket port, path, and TLS next to broker identity; Live copies host, credentials, topic prefix, and device ID from the device MQTT settings.
- **BLE setup no longer shows a diagnostics or sensing-control surface**. The device banner and header dropdown label chip, device ID, and firmware version instead of the Native frontend name. Runtime sensing controls and diagnostics are available through the Device console only after MQTT is live.
...
- **The Device console and header meter subscribe to the complete per-device MQTT topic tree**, including canonical telemetry, command acknowledgements, command catalog, info, OTA state, diagnostics, and Home Assistant state mirrors.
