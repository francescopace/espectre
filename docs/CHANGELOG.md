...
- **Dataset quality validation defaults to writing the markdown report and refreshing pair metadata**: `validate_dataset_quality` regenerates explicit static_presence/motion pairs on every run, updates `dataset_info.json` / `updated_at` only when content changes, and writes `DATASET_QUALITY_CHECK.md` unless `--no-report` is set; `--refresh-metadata`, `--strict`, and `--report` were removed. The report leads with the summary/domains, puts Validation rule last, and Presence/Empty share one `Breath` ladder with Empty soft marks inverted.
- **Dataset `Breath` soft marks use one score**: Presence/Empty rename `Resp` to `Breath`, drop the separate `Breath Hz` column, and fold segment coverage plus quiet-adult human-rate Hz weight into `Breath` (`×1.0` in `0.17-0.33 Hz`, `×0.75` in the `0.13-0.40 Hz` fringe, `×0.40` outside); soft marks still gate on `Breath >= 50` / `Breath >= 35`, with Empty polarity inverted. The wider `0.10-0.50 Hz` window remains the spectral search band only.
