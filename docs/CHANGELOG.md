...
- **Production ML now ships the promoted compact phaseless model**: the exported feature set is seven scale-invariant signals, combining five turbulence/L1 invariants with the two channel-shape dynamics that survived joint ablation and a ten-trial seed search. The runtime removes both the phase and delay-compensated coherence trackers from ML inference, exports the `7 -> 24 -> 12 -> 1` MLP with 505 parameters, and passes all 14 paired plus five quiet deployment replays with no effective alarms. Reserved out-of-sample replay reaches `100%` minimum recall, at most `0.4%` FP, and no effective alarms across C3, C5, C6, and S3. See [FEATURES.md](FEATURES.md).
...
- **The website now publishes a dedicated product roadmap**: the `#roadmap` SPA route and canonical `/roadmap/` page share one CI-generated content source, expose honest Available, Limited validation, Planned, Conditional, and Research states, and present the ordered v3.1-v3.4 product sequence without turning the evidence-gated v3.5 candidate into a release promise. Strategic home-page references connect embedded products, optional local/self-hosted/managed web orchestration, 5 GHz and PHY research, breathing-related motion, and brief gestures to that roadmap.
