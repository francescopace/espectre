...
- **The `./espectre` CLI covers the firmware lifecycle across supported frontends**. It builds, flashes, deploys, monitors, diagnoses, and collects from one command surface. Native, Streamer, ESPHome, and Matter now publish one `_espectre._tcp.local.` Direct HTTP contract with an explicit frontend field, and `./espectre devices` discovers them through one fresh PTR browse without depending on upstream ESPHome or Matter records. Results return after a 350 ms quiet window once a complete record arrives; the 2.5-second default remains the upper bound when no device responds. `collect` reads Streamer's UDP pacing port from the same record and keeps raw CSI on its existing UDP path. The interactive MQTT client discovers devices from retained `info` and `status` topics, selects among multiple matches, and uses each device's published command catalog for help, completion, and command execution.
- **The new [espectre.dev](https://espectre.dev) site brings the device workflow to the browser**. It flashes published firmware, provisions Native Wi-Fi through standard Improv Serial, configures and monitors Native devices over Direct HTTP, monitors through MQTT WebSockets when a broker is configured, manages OTA updates, and hosts the motion game and Wi-Fi Theremin.
...
- **Improv Serial owns Native Wi-Fi credentials, while Direct HTTP owns optional BSSID selection and removal**. Configure reports the current SSID and active band as read-only values, scans access points for the provisioned SSID, displays BSSID and RSSI, and can pin one result or return to automatic selection. The scan keeps the station associated but briefly pauses sensing; a changed pin is verified before it becomes durable and finishes with one automatic reboot. Direct can clear the provisioned network but does not expose password, channel, or band controls.
- **Direct HTTP is now the common local control plane for the C++ frontends**. ESPHome republishes its Home Assistant entity state after Direct runtime writes, Matter uses Direct for detector selection and tuning that its occupancy clusters do not expose, and Streamer exposes bounded status and diagnostics while retaining UDP for raw CSI.
- **Native begins the raw CSI migration with an explicit C3-only Direct capability**. One owner-bound binary collector receives paced V8 records from the existing Direct server while JSON control remains responsive, MQTT remains connected but quiet, and the runtime bypasses sampling and detection. Absolute device-side deadlines prevent worker overhead from reducing the requested cadence, and a temporary external UDP listener can accept an independent host traffic source without coupling it to the HTTP sender. Saved NPZ provenance records the explicit collector target rather than inferring it from observed throughput. The browser shows one selectable live visualization at a time across normalized amplitude, baseline-relative motion, channel waterfall, motion spectrum, channel-shape comparison, and experimental sanitized phase views. The host collector requires `--transport udp|http`, accepts both Streamer V7 and Native V8, records transport provenance, and never falls back silently. Streamer remains the baseline for every chip until the per-chip hardware gates pass.
- **Peer-assisted local discovery now finds every advertised C++ frontend by default**. Every portal attempt creates a new 96-bit Web Crypto nonce and resolves `espectre-devices-<24 hex>.local`; Native answers the matching IPv4 A question statelessly through the existing mDNS socket, avoids reuse of a browser-cached bootstrap address, and returns an NSEC no-AAAA assertion so dual-stack resolvers do not wait for an IPv6 timeout. The single bootstrap POST has a 10-second client bound. Its bounded browse returns Native, Streamer, ESPHome, and Matter endpoints with their advertised ports. The portal negotiates the selected device's exact Direct capabilities and hides unsupported configuration and runtime controls. The published Direct transport boundary supports optional deferred request completion without breaking synchronous SDK implementations, while manual IP, device-ID, remembered-device, Improv, QR, and CLI paths remain available.
- **ESP32-S2 is restored for ESPHome, Native, and Streamer**. CI, release manifests, CLI targets, web-flash artifacts, and current documentation include S2 where the frontend can provision without Bluetooth; Matter deliberately produces no S2 artifact.
...
- **The supported build and hardware baseline changed**. ESPHome requires at least `2026.7.0`, host and ML workflows require Python `3.14`, ESP-IDF integrations require ESP-IDF `>= 5.5`, and PlatformIO-backed builds are no longer supported. ESP32-S2 is supported by ESPHome, Native, and Streamer, but not Matter.
