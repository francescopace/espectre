...
- **Native is a new standalone frontend**. It is BLE-free ESP-IDF firmware. Sensing uses Direct HTTP. MQTT and Home Assistant MQTT Discovery are optional. OTA uses HTTPS. Provisioning uses standard Improv Serial.
- **Matter is a new production frontend**. It publishes the standard occupancy sensor type. Onboarding data is generated per device, and the persisted QR code is available from serial output, the web flasher, or `./espectre matter qr`. Sensing and Direct start only after commissioning. Controller validation remains limited.
- **Micro-ESPectre is a small, read-only sensing frontend**. The deployed application uses Lightweight Detector, the native ICMP generator, five Direct HTTP queries plus session-only recalibration, one telemetry SSE stream, mDNS discovery, and serial logging. Its Direct profile uses strict production Origins, a 10-second SSE heartbeat, a 512-byte request limit, a 4,096-byte event limit, one client, and one frame in flight. The High Accuracy ML model remains available in Python for host research and C++/Python validation but is not copied to the device.
...
- **One versioned protocol covers Direct HTTP and optional MQTT**. ESPectre frontends expose Direct HTTP on port `62587`, advertise `_espectre._tcp.local.`, and enforce the methods declared by their capability profiles. Every frontend derives the same `ESPectre <chip> <short-id>` device name and keeps its optional user label empty until configured. ESPHome keeps this surface enabled by default and can disable Direct HTTP, its discovery records, and browser bootstrap with `direct_api: false`. Supported detector, motion-hit, and traffic selections persist across reboot on the ESP-IDF frontends, while thresholds remain session-only. ESPHome republishes entity state after Direct writes, and Matter uses Direct for controls that its standard occupancy clusters do not expose.
