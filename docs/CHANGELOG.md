...
- **BLE periodic status logs now mirror ESPHome plus MQTT rate**: the standalone BLE frontend now uses the same progress-bar periodic log format as the ESPHome frontend (`mvmt`, `thr`, state, `pkt/s`, channel, RSSI) and appends the observed MQTT publish rate as an extra `msg/s` field.
