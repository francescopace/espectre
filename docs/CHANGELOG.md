...
- **Direct WebSocket is now the common local control plane for the C++ frontends**. ESPHome republishes its Home Assistant entity state after Direct runtime writes, Matter uses Direct for detector selection and tuning that its occupancy clusters do not expose, and Streamer exposes bounded status and diagnostics while retaining UDP for raw CSI.
- **Peer-assisted local discovery now finds every advertised C++ frontend by default**. Every portal attempt creates a new 96-bit Web Crypto nonce and resolves `espectre-devices-<24 hex>.local`; Native answers the matching IPv4 A question statelessly through the existing mDNS socket, avoiding reuse of a browser-cached bootstrap address. Its bounded browse returns Native, Streamer, ESPHome, and Matter endpoints with their advertised ports. The portal negotiates the selected device's exact Direct capabilities and hides unsupported configuration and runtime controls. The published Direct transport boundary supports optional deferred request completion without breaking synchronous SDK implementations, while manual IP, device-ID, remembered-device, Improv, QR, and CLI paths remain available.
