...
- **Managed CSI traffic is paced consistently across firmware and host tools**. Generators avoid catch-up bursts and request low-latency network treatment. Ping remains the universal default, while DNS mode uses a persistent, non-blocking TCP connection to avoid the unfavorable handling observed with UDP/53.
