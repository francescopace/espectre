...
`./espectre collect` persistently selects `external`, opens the bearer-bound raw session, and imports `ExternalTrafficGenerator` from the standalone, standard-library-only `tools/ha_traffic_generator_addon/espectre_traffic_generator.py`. Its `--pps` value controls only that UDP generator and dataset provenance. External datagrams carry the exact four-byte UTF-8 payload `"👻".encode("utf-8")` (`F0 9F 91 BB`) as the canonical marker. The web raw tool uses the device's existing internal or external configuration and does not expose a PPS control.
