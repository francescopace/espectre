...
For continuous external traffic on 64-bit Home Assistant OS, use the **ESPectre Traffic Generator** add-on. It runs the shared UDP generator with Home Assistant controls for configuration, start, and stop. Set each device to `csi_traffic_mode: external` and match the add-on's `rate_pps` to the device's `csi_target_pps`; the add-on does not change device settings. See [DOCS.md](../tools/ha_traffic_generator_addon/DOCS.md) for installation, requirements, and configuration. Stop the add-on before running `./espectre collect` for the same devices, because collection starts its own generator.
