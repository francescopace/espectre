...
- Micro-ESPectre owns a single `_espectre._tcp` advertisement for its bounded Direct surface. It exposes monitoring queries, telemetry, and session-only manual recalibration, but no configuration mutations, raw CSI stream, or peer-discovery responder.
