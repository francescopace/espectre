<!-- Generated file. Do not edit manually. -->

# Motion Detection Performance

Last update: 2026-08-13
Source: `data/dataset_info.json`
Dataset revision: `sha256:6fd0edc261f4132f563c0ce82a1bf2403ffe5b73637957614be8cc817aae79ee`
Input revision: `sha256:bf6208d9d795647a560828f9e1baacf53bd9b14365dc87d0f5f275df8bed7811`
Generated by: `tools/generate_performance_report.py`
Run started: `2026-08-13T13:38:50+02:00`
Run duration: `6.57s`
Inputs: `13` real paired datasets, `0` synthetic paired datasets, `8` long quiet datasets

This report shows how ESPectre's motion detectors perform on recorded data.

`tools/generate_performance_report.py` also verifies that the host-side C++ and Python implementations produce matching results when they replay the same recordings.

- **Classic Detector**: Analyzes changes in the Wi-Fi signal and calibrates itself for each recording. It is not trained on recorded data.
- **ML Detector**: Uses a neural network trained on recordings marked as motion or no motion. Published replay results use only recordings outside its training corpus.

See [ALGORITHMS.md](../ALGORITHMS.md) for the full detector design.

---

## Current Host Resource Benchmark

The generator compiles the current production C++ sources once per source revision and executes the benchmark on every report run. Timings and CPU estimates therefore describe the report host, not an ESP device. Persistent memory is the detector object's `sizeof` plus live heap allocations after construction; allocator metadata is not included.

Nominal load: `100` packets/s and `4` inferences/s; window `100` packets.

| Detector | Persistent memory | Packet median / p90 | Inference median / p90 | Modeled detector CPU | Transient heap |
| --- | ---: | ---: | ---: | ---: | ---: |
| Classic | 1,792 B | 0.09 / 0.09 us | 0.74 / 0.77 us | 12.15 us/s | 0 B |
| ML | 6,008 B | 0.45 / 0.45 us | 1.82 / 1.92 us | 51.94 us/s | 0 B |

---

## Required Performance

| Metric | Target | 
|-------|--------|
| Recall | >95% |
| FP Rate | <5% |

Recall is the percentage of motion correctly detected. FP Rate is the percentage of no-motion readings incorrectly reported as motion.

---

## Tests Included

- C++ `test_motion_detection`
- C++ `test_long_recordings`
- Python `TestPerformanceMetrics`
- Python `test_validation_long_recordings.py`

---

## Normal Wi-Fi Signal

Effective Alarms counts how many times the detector incorrectly switches to MOTION when no motion is present. It uses the same filtering as the firmware.

These recordings contain periods without motion followed by periods with motion, captured with a normal Wi-Fi signal. Both detectors are reported only on the combined `selection + holdout` corpus.

### Classic Detector

| Metric | ESP32-C3 | ESP32-C5 | ESP32-C6 | ESP32 | ESP32-S3 |
|--------|----------|----------|----------|----------|----------|
| Recall | 99.7% | 100.0% | 100.0% | N/A | 100.0% |
| Min Recall | 99.4% | 100.0% | 100.0% | N/A | 100.0% |
| Precision | 99.7% | 96.7% | 97.8% | N/A | 98.9% |
| FP Rate | 0.1% | 1.7% | 1.1% | N/A | 0.6% |
| Max FP Rate | 0.1% | 1.7% | 1.1% | N/A | 0.6% |
| F1-Score | 99.7% | 98.3% | 98.9% | N/A | 99.4% |
| Effective Alarms | 0 | 0 | 2 | N/A | 0 |

### ML Detector

| Metric | ESP32-C3 | ESP32-C5 | ESP32-C6 | ESP32 | ESP32-S3 |
|--------|----------|----------|----------|----------|----------|
| Recall | 100.0% | 100.0% | 100.0% | N/A | 100.0% |
| Min Recall | 100.0% | 100.0% | 100.0% | N/A | 100.0% |
| Precision | 100.0% | 100.0% | 100.0% | N/A | 100.0% |
| FP Rate | 0.0% | 0.0% | 0.0% | N/A | 0.0% |
| Max FP Rate | 0.0% | 0.0% | 0.0% | N/A | 0.0% |
| F1-Score | 100.0% | 100.0% | 100.0% | N/A | 100.0% |
| Effective Alarms | 0 | 0 | 0 | N/A | 0 |

---

## Reserved Augmentation Diagnostic

Classic and ML are evaluated on the same reserved `selection + holdout` pairs after deterministic `base+drift+burst-loss` packet augmentation. As in production ML training, the fixed views `20260807, 20260808` contribute alternating row positions. These combined reserved results are diagnostic, not an uncontaminated promotion gate.

| Detector | Corpus | Recall | FP Rate | F1 |
| --- | --- | ---: | ---: | ---: |
| Classic | Reserved, mixed two-seed augmentation (13 pairs) | 98.6% | 0.7% | 98.6% |
| ML | Reserved, mixed two-seed augmentation (13 pairs) | 99.1% | 0.1% | 99.5% |

---

## Weak Wi-Fi Signal

These reserved `selection + holdout` recordings show detector behavior when the Wi-Fi signal is weak. The ML requirements for this test are recall >90% and FP <10%; Classic results are included for information only.

### Classic Detector

| Metric | ESP32-C3 | ESP32-C5 | ESP32-C6 | ESP32 | ESP32-S3 |
|--------|----------|----------|----------|----------|----------|
| Recall | 99.3% | 98.1% | 100.0% | N/A | 99.7% |
| Min Recall | 98.6% | 96.3% | 100.0% | N/A | 99.4% |
| Precision | 99.7% | 95.5% | 98.9% | N/A | 98.1% |
| FP Rate | 0.1% | 2.4% | 0.6% | N/A | 1.0% |
| Max FP Rate | 0.3% | 4.7% | 0.6% | N/A | 1.4% |
| F1-Score | 99.5% | 96.8% | 99.4% | N/A | 98.9% |
| Effective Alarms | 0 | 6 | 1 | N/A | 2 |

### ML Detector

| Metric | ESP32-C3 | ESP32-C5 | ESP32-C6 | ESP32 | ESP32-S3 |
|--------|----------|----------|----------|----------|----------|
| Recall | 99.9% | 98.3% | 100.0% | N/A | 98.4% |
| Min Recall | 99.7% | 97.1% | 100.0% | N/A | 95.7% |
| Precision | 100.0% | 100.0% | 100.0% | N/A | 99.9% |
| FP Rate | 0.0% | 0.0% | 0.0% | N/A | 0.0% |
| Max FP Rate | 0.0% | 0.0% | 0.0% | N/A | 0.1% |
| F1-Score | 99.9% | 99.1% | 100.0% | N/A | 99.1% |
| Effective Alarms | 0 | 0 | 0 | N/A | 0 |

---

## Long Quiet Reserved Recordings

These `selection + holdout` recordings contain no motion. They show how often each detector reports motion during quiet periods.

### Classic Detector

| Metric | C3 | C5 | C6 | ESP32 | S3 |
|--------|----|----|----|----|----|
| Avg FP Rate | 0.04% | 0.39% | 1.81% | N/A | 0.83% |
| Max FP Rate | 0.09% | 0.39% | 4.39% | N/A | 1.67% |
| Effective Alarms | 0 | 0 | 11 | N/A | 4 |

### ML Detector

| Metric | C3 | C5 | C6 | ESP32 | S3 |
|--------|----|----|----|----|----|
| Avg FP Rate | 0.00% | 0.00% | 0.35% | N/A | 0.21% |
| Max FP Rate | 0.00% | 0.00% | 0.90% | N/A | 0.43% |
| Effective Alarms | 0 | 0 | 0 | N/A | 0 |
