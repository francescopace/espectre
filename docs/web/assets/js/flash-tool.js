...
'use strict';
    ...
    const FLASH_MATTER_PROBE_TIMEOUT_MS = 10000;
    ...
    async function flashDetect(reusePort = false) {
        ...
        try {
            ...
            let matterInfo = null;
            if (!improv) {
                flashNotifyTransition('detecting', 'Checking for Matter firmware…');
                matterInfo = await flashProbeMatter();
                if (operation !== flash.operation) return;
            }
            const reportedChip = flashNormalizeChip(
                improv?.info?.chipFamily || matterInfo?.chipFamily || ''
            );
            let chip = reportedChip;
            if (!flashAnyFrontendSupportsChip(reportedChip)
                    || (matterInfo && !matterInfo.version)) {
                flashNotifyTransition('detecting', 'Identifying the connected board…');
                chip = await flashEnterLoader(api);
            }
            if (!improv && (!matterInfo || !matterInfo.version) && flash.loader) {
                const storedInfo = await flashReadInstalledMatterInfo(flash.loader, chip);
                if (storedInfo) {
                    matterInfo = storedInfo;
    ...
    async function flashReadInstalledMatterInfo(loader, chipFamily) {
        try {
            ...
            for (const partition of appPartitions) {
                ...
                if (descriptor?.projectName.toLowerCase() === 'espectre-matter') {
                    return {
                        firmware: 'ESPectre Matter',
    ...
    function flashMatterIdentity(input) {
        const project = input.match(/Project name:\s*([^\r\n\x1B]+)/i)?.[1]?.trim() || '';
        const version = input.match(/App version:\s*([^\r\n\x1B]+)/i)?.[1]?.trim() || '';
        const runtimeMarker = /(?:MATTER_QR=|ESPectre Matter firmware started)/i.test(input);
        if (project.toLowerCase() !== 'espectre-matter' && !runtimeMarker) return null;
        if (!version && !runtimeMarker) return null;
    ...
    async function flashReadMatterSerial(timeoutMs, match) {
        await flashOpenPort('matter');
    ...
    async function flashProbeMatter(timeoutMs = FLASH_MATTER_PROBE_TIMEOUT_MS) {
        const info = await flashReadMatterSerial(timeoutMs, flashMatterIdentity);
    ...
    async function flashReadMatterInput(timeoutMs = 20000) {
        ...
        const result = await flashReadMatterSerial(timeoutMs, (input) => {
            const codes = flashMatterCodes(input);
            return codes ? { ...codes, info: flashMatterIdentity(input) } : null;
        });
    ...
    window.ESPectreFlashCore = Object.freeze({
        ...
        matterIdentity: flashMatterIdentity,
        matterCodes: flashMatterCodes,
        appDescriptor: flashAppDescriptor,
        readInstalledMatterInfo: flashReadInstalledMatterInfo,
