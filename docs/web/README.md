...
The SDK landing page recommends ESP-IDF Component Manager and keeps source archives as an alternative. The production registry contains stable SDK releases; staging contains tagged prereleases, Preview snapshots from `main`, and Develop snapshots from `develop`. Website Release can point to a prerelease and therefore does not always map to production. Registry snapshot versions include a branch suffix that archive labels omit. See [SDK.md](../SDK.md#esp-component-registry) for installation commands.
