...
SDK package links select the API reference with `?sdk=<version>&commit=<full-sha>`. The browser loads that revision from `artifacts/sdk/api/revisions/<commit>/<version>/`, and keeps the selection when opening another symbol. An unavailable or mismatched revision shows an error. The unqualified API URL opens the reference from the current website build. Generation checks that every public header in the Doxyfile produces a rendered page; detailed integration contracts come from `src/cpp/sdk_integration.dox`.

Pass `--archive-dir <directory>` to retain the generated `sdk-api-<version>-<commit>.zip`. CI publishes this archive alongside the SDK bundles. Identical uploads are skipped, and conflicting contents fail publication. Keep these assets on tagged and rolling GitHub releases: the Pages deployment restores them to preserve existing package links.
...
Snapshot and Release dispatch `pages.yml` on `main` and wait for its result. That workflow validates the source publication run and downloads its verified Pages archive. Within the serialized deployment job, it restores SDK API archives from published GitHub releases, validates their identities and HTML, and adds their immutable revision directories. It rejects conflicting revisions and preserves the other website files. After deployment, the channel's firmware catalog, SDK catalog, and signing-key registry must match the archive byte for byte before IndexNow is notified. A stale deployment fails after a bounded propagation wait.
