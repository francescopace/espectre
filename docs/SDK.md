...
The build matrix covers sensing on all six targets, each optional service group and all groups together on C3, and the complete configuration on S2 with TinyUSB. Build coverage does not establish sensing quality for every board or radio environment. See [README.md](performance/README.md) for measured results.
...
Each group is off by default, so a minimal integration does not link transport code it never calls. Enabling frontend support also selects provisioning because its bootstrap helpers call that service. ESP-IDF resolves component requirements before menuconfig. The ESP-IDF component therefore declares every optional stack dependency up front, while the source switches control what reaches the firmware image. The manifest pins `espressif/mdns` to `1.12.0` and resolves `espressif/esp_tinyusb` only for ESP32-S2. External components retain their own licenses. Source-list integrations declare their selected stack dependencies themselves.
...
The production ESP Component Registry publishes stable releases. SDK snapshots from `main` and `develop`, along with release candidates, are published to the [staging registry](https://components-staging.espressif.com/). Registry snapshots use immutable versions containing their channel and source commit; their manifest, SDK version macros, and example dependency identify that registry version. See [RELEASING.md](RELEASING.md#snapshot-workflow) for the version format and instructions for selecting an exact staging dependency.
...
Maintainer procedures for packaging and publication are documented in [RELEASING.md](RELEASING.md#sdk-registry-publishing).
...
The public SDK is `GPL-3.0-only`, which permits commercial use subject to GPLv3's terms and applicable corresponding-source obligations. A separate commercial license is available for proprietary firmware. The package includes `LICENSE`, `LICENSING.md`, and `THIRD_PARTY_NOTICES.md`; external dependencies retain their own licenses. See [LICENSING.md](../LICENSING.md).
