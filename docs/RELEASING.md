...
3. In the Espressif registries, configure trusted uploaders for repository `francescopace/espectre`: two staging uploaders (`snapshot.yml` and `release.yml`, both with environment `sdk-registry-staging`) and one production uploader (`release.yml`, with environment `sdk-registry-production`). In each Espressif trusted uploader, leave the Branch field empty; GitHub environment rules restrict the allowed refs. Each publisher's filename identifies the workflow that requests the OIDC token. Replace the previous `ci.yml` trusted uploaders when migrating from reusable publication workflows. Follow Espressif's [OIDC setup](https://docs.espressif.com/projects/idf-component-manager/en/latest/publish/how_to_github_actions_upload.html), and verify the uploader configuration with the first staging run.
...
Release tag candidates must pass the existing semantic-version and finalized-changelog checks, and their commits must be reachable from `main`, before CI can use publication signing keys. The tag run supplies the required CI checks. Stable publication also requires successful SDK staging verification from a `main` snapshot of the exact same commit.
...
On a validated release tag, CI builds firmware and SDK artifacts with the exact tag as their version. After all checks pass, it dispatches `release.yml` on that tag with the CI run ID, attempt, and full commit SHA. The release workflow verifies the completed source CI attempt and its successful aggregate check and dispatch jobs, revalidates the tag and main ancestry, and consumes those artifacts.

For stable tags, `Verify Main Staging` requires a completed Snapshot run from this repository on `main`, at the tag's exact commit, with a successful `SDK Staging Verified` job. A `develop` snapshot, a different commit, or a skipped staging check cannot authorize publication. Failed-job retries can reuse an earlier successful staging check within the same run, unless a newer attempt of that check failed or was skipped. Failures in unrelated snapshot jobs do not invalidate successful SDK verification. The gate waits up to 45 minutes if the matching snapshot has not appeared or is still running, and fails if all matching runs have finished without successful staging verification. Complete the main snapshot and retry the failed Release jobs if this gate fails.

Stable tags skip their own staging upload and matrix. After the main staging gate and GitHub release succeed, the production job uploads the final-version archive already built and verified by tag CI, using the pinned official Espressif action and OIDC. A separate verification job downloads and compares the public component and example, then builds a clean C3 consumer with all optional groups.

Prerelease tags, such as `3.0.0-rc1`, upload their CI component archive to staging and run the complete registry verification matrix before creating the GitHub release. The matrix resolves the exact prerelease SDK version from staging, with other dependencies coming from production, and checks installed contents after compilation. Prereleases never upload to production. Both registry upload paths compare an existing version before skipping upload; differing contents fail the run. Verification downloads have a bounded wait, so a delayed upload can be retried without silently accepting a different package.
...
Before creating a stable tag, let a main snapshot containing `SDK Staging Verified` complete successfully on that exact commit. Older snapshot runs without this explicit check cannot authorize stable publication.
