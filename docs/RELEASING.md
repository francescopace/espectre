...
The packager copies an explicit set of SDK files and stamps the same version into the manifest, SDK macros, and example dependency. It records the source commit and validates the manifests with `compote`.
...
2. Configure GitHub environments `sdk-registry-staging` and `sdk-registry-production`. Allow release tags (`*.*.*`) in both, add **Branch** rules for `main` and `develop` in staging, and require owner approval for the first production release. Add one trusted uploader in each registry for repository `francescopace/espectre` and workflow `ci.yml`: use environment `sdk-registry-staging` in staging and `sdk-registry-production` in production. Leave the Branch fields empty; GitHub environment rules restrict the allowed refs. GitHub's [OIDC claims for reusable workflows](https://docs.github.com/en/actions/how-tos/secure-your-work/security-harden-deployments/oidc-with-reusable-workflows) identify the caller as `ci.yml` for both snapshots and releases, so replace existing `snapshot.yml` or `release.yml` uploaders with `ci.yml`. The rolling tags `snapshot` and `snapshot-dev` are publication outputs; their environment rules are unnecessary. Follow Espressif's [OIDC setup](https://docs.espressif.com/projects/idf-component-manager/en/latest/publish/how_to_github_actions_upload.html), and verify the uploader configuration with the first staging run.
...
On pushes to `main` or `develop`, the final `publish-snapshot` job in `ci.yml` waits for every build, test, audit, and SDK component check, then calls the reusable `snapshot.yml` from the same commit. Firmware, GitHub releases, website updates, and registry uploads require all prerequisite jobs to succeed. Failed checks still allow available coverage badges to update on the existing rolling release. Pull requests, other branches, and manually dispatched CI runs do not publish snapshots.

The called workflow verifies the caller's CI run, branch, and commit, downloads its existing `registry-component` artifact, and uploads it to staging through OIDC. It checks that the source commit is still current, that the archive and inventory agree, and that the component belongs to the validated commit and channel. A push to `develop` publishes `snapshot-dev`; a push to `main` publishes `snapshot` and updates the website. Both branches upload SDK snapshots to staging.

To retry a failed publication, use **Re-run failed jobs** on the original push CI run while its artifacts are retained. The original branch and commit remain the publication source, and superseded commits are skipped. Push CI runs on these branches and on tags finish without automatic cancellation by newer runs so an upload or tag update can complete; pending runs may be superseded. Other CI runs retain automatic cancellation.
...
On a validated release tag, CI builds firmware and SDK artifacts with the exact tag as their version. After all checks pass, it calls `release.yml`, which verifies the caller and consumes those artifacts. It uploads the existing component archive to staging using the pinned official Espressif action and OIDC, waits for processing, and compares every downloaded file with the inventory.
...
Retry a failed release publication through **Re-run failed jobs** on the original tag CI run while its artifacts are retained. The website deployment retains a separate automatic dispatch of `release.yml` on `main` to avoid the existing Pages issue with tag deployments reusing an older artifact for the same commit. That dispatch verifies the source CI run and completed release jobs, then deploys the exact Pages archive from the successful website build attempt. A manual website-only retry accepts the tag CI run ID as `pages_run_id` and does not publish firmware or registry components.
