...
Micro-ESPectre uses MicroPython-friendly rate and performance samplers to cache the canonical fields it can measure. Its read-only Direct `diagnostics` query exposes that cache; unavailable C++ measurements are omitted, and diagnostic collection emits no periodic serial log.
