...
Micro-ESPectre exposes read-only Direct snapshots and telemetry, but it does not run the continuous production diagnostics sampler used by the C++ frontends. Its default-off `DEBUG_TELEMETRY` switch enables machine-readable serial timing, heap, garbage-collection, and packet-processing measurements for benchmark runs.
