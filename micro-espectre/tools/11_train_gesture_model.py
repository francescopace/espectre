#!/usr/bin/env python3
"""
LogReg Gesture Detection - Training Script

Trains a logistic-regression gesture classifier from labeled CSI data collected with labels
...
classifier to learn the temporal morphology of each gesture (rise time, peak shape,
fall time, etc.).
...
def _build_model_estimator(model_family='logreg', random_seed=42):
    """Build sklearn estimator/pipeline for the selected model family."""
...
def _fit_model(X, y, model_family='logreg', random_seed=42):
    """Fit selected model family on feature matrix."""
    model = _build_model_estimator(
...
def _decision_to_probabilities(decision_values, n_classes):
    """Convert decision_function outputs to pseudo-probabilities."""
    dec = np.asarray(decision_values, dtype=np.float64)
    ...
    ex = np.exp(logits)
    den = np.sum(ex, axis=1, keepdims=True) + 1e-12
    return ex / den


def _predict_model_proba(model, X, n_classes):
    """Return class probabilities (or robust approximations) for threshold tuning."""
    ...
    if hasattr(model, 'decision_function'):
        proba = _decision_to_probabilities(model.decision_function(X), n_classes)
        if proba is not None:
            return proba

    # Fallback: one-hot probabilities from hard predictions.
    pred = np.asarray(model.predict(X), dtype=np.int32)
    proba = np.zeros((len(pred), n_classes), dtype=np.float64)
    proba[np.arange(len(pred)), pred] = 1.0
    return proba


def _cross_validate_model(X, y, groups=None, n_folds=5, model_family='logreg',
                          random_seed=42):
    """Cross-validate selected model family with optional group split."""
    ...
    base_model = _build_model_estimator(
        model_family=model_family,
        random_seed=random_seed,
    )
...
def _resolve_feature_names(feature_preset='baseline'):
    ...
    if feature_preset == 'reduced_plus_paper_plus_exp':
        base = _resolve_feature_names('reduced_plus_paper') + list(EXPERIMENTAL_GESTURE_FEATURES)
...
def _logreg_export_arrays(model):
    """Extract scaler + logistic regression arrays."""
    ...
    return {
        'scaler': scaler,
        'num_classes': int(coef.shape[0]),
        'num_features': int(coef.shape[1]),
        'coef': coef,
        'intercept': intercept,
    }


def _export_logreg_micropython(model, output_path, seed, class_names, feature_names,
                               reject_confidence, reject_margin):
    """Export LogReg parameters for MicroPython gesture inference."""
    data = _logreg_export_arrays(model)
    scaler = data['scaler']
...
Architecture: LogReg ({data['num_features']} -> {data['num_classes']})
Trained: {timestamp}
Seed: {seed}
...
GESTURE_NUM_FEATURES = {data['num_features']}
...
GESTURE_LOGREG_COEFS = [{', '.join(f'{v:.8f}' for v in data['coef'].ravel(order='C').tolist())}]
GESTURE_LOGREG_INTERCEPTS = [{', '.join(f'{v:.8f}' for v in data['intercept'].tolist())}]
'''
...
def _export_logreg_cpp(model, output_path, seed, class_names, feature_names,
                       reject_confidence, reject_margin):
    """Export LogReg parameters for C++ ESPHome gesture inference."""
    data = _logreg_export_arrays(model)
    scaler = data['scaler']
    timestamp = __import__('datetime').datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
constexpr int GESTURE_NUM_FEATURES = {data['num_features']};
...
'''
    for name in class_names:
        code += f'    "{name}",\n'
    code += '};\n'
    code += f'constexpr uint8_t GESTURE_FEATURE_IDS[{data["num_features"]}] = {{{", ".join(str(v) for v in _feature_ids(feature_names))}}};\n\n'

    code += f'constexpr float GESTURE_FEATURE_MEAN[{data["num_features"]}] = {{{", ".join(f"{x:.8f}f" for x in scaler.mean_)}}};\n'
    code += f'constexpr float GESTURE_FEATURE_SCALE[{data["num_features"]}] = {{{", ".join(f"{x:.8f}f" for x in scaler.scale_)}}};\n\n'

    code += f'constexpr float GESTURE_LOGREG_COEFS[{data["num_classes"] * data["num_features"]}] = {{{", ".join(f"{v:.8f}f" for v in data["coef"].ravel(order="C").tolist())}}};\n'
    code += f'constexpr float GESTURE_LOGREG_INTERCEPTS[{data["num_classes"]}] = {{{", ".join(f"{v:.8f}f" for v in data["intercept"].tolist())}}};\n\n'
...
def train_all(seed=None, window_seconds=2.0, window_overlap=0.0,
              ...
              validation_summary=None,
              print_validation_summary=True):
    """Train gesture classifier (LogReg) with selected feature preset.
    ...
    if n_folds < 2:
        ...
        print(f'{n_folds}-fold cross-validation (LogReg {len(feature_names)}->{num_classes})...')
        cv_results = _cross_validate_model(
            X, y, groups=groups, n_folds=n_folds, model_family='logreg',
    ...
    model = _fit_model(X, y, model_family='logreg', random_seed=seed or 42)
    ...
    proba = _predict_model_proba(model, X_test_raw, n_classes=len(class_names))
    ...
    mp_size, _ = _export_logreg_micropython(
...
def run_sequential_train_search(max_runs=12,
    ...
    print(f'Train config: model=logreg, preset={feature_preset}')
...
'''
    ...
    parser.add_argument('--feature-preset', choices=FEATURE_PRESET_CHOICES,
