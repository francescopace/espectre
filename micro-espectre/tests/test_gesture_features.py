...
class TestPhaseFeatures:
    ...
    def test_inter_sc_coherence_range(self):
        phases = [[0.0, 0.2, 0.4, 0.6, 0.8] for _ in range(20)]
        coh = calc_phase_inter_sc_coherence(phases)
        assert 0.0 <= coh <= 1.0
...
class TestExtractGestureFeatures:
    ...
    def test_experimental_feature_names_non_empty(self):
        assert len(EXPERIMENTAL_GESTURE_FEATURES) >= 1
    ...
    def test_event_without_phases(self):
        ...
        assert features[14] == pytest.approx(0.0)
        assert features[15] == pytest.approx(0.0)
    ...
    def test_gaussian_event(self):
        ...
        # Feature-specific sanity checks (index mapping aligned with extractor)
        assert math.isfinite(features[9])      # turb_mid_mean
        assert math.isfinite(features[10])     # turb_late_mean
        assert math.isfinite(features[11])     # turb_late_minus_mid (can be negative)
        assert features[12] >= 0.0             # turb_diff_abs_mean
        assert features[13] >= 0.0             # phase_diff_var
        assert features[14] >= 0.0             # phase_entropy
        assert 0.0 <= features[15] <= 1.0      # phase_circular_variance

    def test_known_turb_diff_abs_mean(self):
        event = [{'turbulence': float(i), 'phases': [0.0, 0.0, 0.0]} for i in range(5)]
        features = extract_gesture_features(event)
        assert features[12] == pytest.approx(1.0, abs=1e-10)

    def test_late_minus_mid_can_be_negative(self):
        # middle third high, late third low -> late_minus_mid < 0
        turb = [1.0, 1.0, 1.0, 10.0, 10.0, 10.0, 1.0, 1.0, 1.0]
        event = [{'turbulence': t, 'phases': [0.0, 0.0, 0.0]} for t in turb]
        features = extract_gesture_features(event)
        assert features[11] < 0.0

    def test_phase_entropy_uniform_bins(self):
        # 5 equally populated bins -> entropy = log2(5)
        phases = [0.0, 1.0, 2.0, 3.0, 4.0]
        event = [{'turbulence': 1.0, 'phases': phases} for _ in range(10)]
        features = extract_gesture_features(event)
        assert features[14] == pytest.approx(math.log2(5.0), abs=0.02)
    ...
    def test_custom_feature_subset_order(self):
        event = [{'turbulence': 1.0 + i * 0.1, 'phases': [0.1, 0.2, 0.3, 0.4]} for i in range(30)]
        selected = ['event_duration', 'phase_entropy', 'phase_inter_sc_coherence']
        out = extract_gesture_features(event, feature_names=selected)
        assert len(out) == len(selected)
        # event_duration still first in selected ordering
        assert out[0] > 0.0
        assert out[1] >= 0.0
