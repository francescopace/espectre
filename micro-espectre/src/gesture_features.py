...
Feature categories:
  - Temporal shape / robust turbulence (13 features)
...
def _segment_mean(arr, start, end):
    if end <= start:
        return 0.0
    seg = arr[start:end]
...
def calc_turb_mid_mean(turb_list):
    """Mean turbulence in middle third of event."""
    n = len(turb_list)
    if n < 3:
        return 0.0
    s = n // 3
    e = (2 * n) // 3
    return _segment_mean(turb_list, s, e)


def calc_turb_late_mean(turb_list):
    """Mean turbulence in late third of event."""
    n = len(turb_list)
    if n < 3:
        return 0.0
    s = (2 * n) // 3
    return _segment_mean(turb_list, s, n)


def calc_turb_late_minus_mid(turb_list):
    """Late-third mean minus middle-third mean turbulence."""
    return calc_turb_late_mean(turb_list) - calc_turb_mid_mean(turb_list)


def calc_turb_diff_abs_mean(turb_list):
    """Mean absolute first difference of turbulence signal."""
    n = len(turb_list)
    if n < 2:
        return 0.0
    diffs = [abs(turb_list[i] - turb_list[i - 1]) for i in range(1, n)]
...
def calc_phase_inter_sc_coherence(phases_list):
    """
    Inter-subcarrier phase coherence proxy.

    For each packet compute variance of wrapped adjacent phase differences.
    Return coherence score = 1 / (1 + mean_var), higher is more coherent.
    """
    if not phases_list:
        return 0.0
    ...
    for phases in phases_list:
        ...
        if n < 3:
            continue
        diffs = [_wrap_phase_delta(phases[i + 1] - phases[i]) for i in range(n - 1)]
        mean_d = sum(diffs) / len(diffs)
...
def extract_gesture_features(event_buffer, feature_names=None):
    """
    ...
    Args:
        feature_names: Optional list of feature names to extract. If None,
            uses default GESTURE_FEATURES (backward-compatible).

    Returns:
        list: Feature vector aligned with requested feature_names
    """
    selected = feature_names if feature_names is not None else GESTURE_FEATURES
    ...
    feature_map = {
        'event_duration': calc_event_duration(turb_list),
        'peak_position': calc_peak_position(turb_list),
        'peak_to_mean_ratio': calc_peak_to_mean_ratio(turb_list),
        'rise_fall_asymmetry': calc_rise_fall_asymmetry(turb_list),
        'pre_post_energy_ratio': calc_pre_post_energy_ratio(turb_list),
        'n_local_peaks': calc_n_local_peaks(turb_list),
        'peak_fwhm': calc_peak_fwhm(turb_list),
        'turb_mad': calc_turb_mad(turb_list),
        'turb_iqr': calc_turb_iqr(turb_list),
        'turb_mid_mean': calc_turb_mid_mean(turb_list),
        ...
        'phase_inter_sc_coherence': calc_phase_inter_sc_coherence(phases_list),
    }

    return [feature_map[name] for name in selected]
