...
class NBVICalibrator:
    ...
    def calibrate(self, current_band, window_size=None, step=None):
        ...
        # Log baseline detection results
        #print(f"NBVI: Baseline window found")
        #print(f"  Variance: {baseline_stats['variance']:.4f}")
        #print(f"  p{self.percentile} threshold: {baseline_stats['threshold']:.4f} (adaptive)")
        #print(f"  Windows analyzed: {baseline_stats['windows_analyzed']}")
        #print(f"  Baseline candidates: {baseline_stats['baseline_candidates']}")
        ...
        selected_band, top_5 = self._select_with_spacing(sorted_metrics, k=12)
        ...
        # Log final results
        #print(f"NBVI: Selected {len(selected_band)} subcarriers")
        #print(f"  With spacing Δf≥{self.min_spacing}: {selected_band[5:]}")
        ##print(f"  Top 5: {top_5}")
