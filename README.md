...
**This project uses a pure mathematical approach** based on the **MVS (Moving Variance Segmentation)** algorithm for motion detection and **P95 Band Selection** for automatic subcarrier optimization.
...
For algorithm details (MVS, P95 Band Selection, Hampel filter), see [ALGORITHMS.md](micro-espectre/ALGORITHMS.md).
...
│ Calibration │  Selects optimal 12-subcarrier band via P95
└──────┬──────┘
       │
       ▼
┌─────────────┐
...
ESPectre implements **P95 Band Selection** for automatic subcarrier optimization, achieving near-optimal performance (F1=98.2%) with **zero manual configuration**. The algorithm selects a contiguous band of 12 subcarriers that minimizes false positive rate.
...
While ESPectre v2.x focuses on **motion detection** (MVS + P95 Band Selection), the project is exploring machine learning capabilities for advanced applications:
...
| [Algorithms](micro-espectre/ALGORITHMS.md) | Scientific documentation of MVS, P95 Band Selection, Hampel filter |
