...
export const peerDiscoveryScenarios = Object.freeze({
    multiFrontend: result([
        peer(),
        peer({
            device_id: '1111111111111111', instance: 'Collector', hostname: 'espectre-1111111111111111',
            name: 'Collector', frontend: 'streamer', chip: 'esp32s3', capabilities: ['collect', 'monitor'],
            addresses: ['192.168.1.43']
        }),
        peer({
            device_id: '2222222222222222', instance: 'ESPHome sensor', hostname: 'espectre-2222222222222222',
            name: 'ESPHome sensor', frontend: 'esphome', chip: 'esp32c6', port: 6054,
            capabilities: ['config', 'monitor'], addresses: ['192.168.1.44']
        }),
        peer({
            device_id: '3333333333333333', instance: 'Matter sensor', hostname: 'espectre-3333333333333333',
            name: 'Matter sensor', frontend: 'matter', chip: 'esp32c3', capabilities: ['config', 'monitor'],
    ...
    mixedAddresses: result([
        ...
        peer({
            ...
            instance: 'Office sensor',
            hostname: 'espectre-fedcba9876543210',
            name: 'Office',
            frontend: 'esphome',
            chip: 'esp32s3',
            port: 6054,
            capabilities: ['monitor'],
            addresses: ['fd12:3456:789a::42']
        })
    ]),
    duplicateIdentity: result([
        peer(),
        peer({ hostname: 'conflicting-host', addresses: ['192.168.1.43'] })
    ]),
    malformed: result([peer({ path: '/espectre/v1/ws?token=x' })]),
