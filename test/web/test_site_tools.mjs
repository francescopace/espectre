...
describe('website tool contracts', () => {
    ...
    it('loads generated firmware and SDK output from the shared artifacts tree', () => {
        ...
        assert.match(nativeNetworkPanel, /espectre&gt; update_sensing threshold=0\.35/);
    ...
    it('maps Direct capabilities, runtime controls, and Wi-Fi access points safely', () => {
        ...
        assert.match(app, /const sections = new Set\(capabilities\.resources \|\| \[\]\)/);
        assert.match(app, /supports_wifi_status: sections\.has\('wifi'\)/);
        assert.match(app, /supports_wifi_bssid: methods\.has\('set_wifi_bssid'\)[\s\S]*?methods\.has\('clear_wifi_bssid'\)[\s\S]*?methods\.has\('scan_wifi'\)/);
        assert.match(app, /supports_wifi_clear: methods\.has\('clear_wifi_credentials'\)/);
        ...
        assert.match(app, /supports_runtime_threshold: methods\.has\('update_sensing'\)/);
        assert.match(app, /supports_runtime_motion_hits: methods\.has\('update_sensing'\)/);
        assert.match(app, /supports_runtime_detector: methods\.has\('update_sensing'\)/);
        assert.match(app, /supports_manual_recalibration: methods\.has\('recalibrate'\)/);
        assert.match(app, /edit\.hidden = false/);
        assert.match(app, /data\.label \|\| data\.name/);
        ...
        assert.match(app, /activeToolName\(\) === 'configure'[\s\S]*?directClient\.request\('get', 'diagnostics'\)/);
        ...
        assert.match(app, /const client = directClient[\s\S]*client\.request\('post', 'wifi\/scans'\)/);
        assert.match(app, /client\.request\('get', 'wifi\/access-points'\)/);
        ...
        assert.match(app, /const action = bssid \? 'set_wifi_bssid' : 'clear_wifi_bssid'/);
        ...
        assert.match(app, /function cfgSaveDeviceLabel\(label\)[\s\S]*?'update_device'/);
        ...
        assert.match(app, /const info = await directClient\.request\('get', 'device'\);[\s\S]*const status = await directClient\.request\('get', 'health'\);[\s\S]*const sensing = await directClient\.request\('get', 'sensing'\);[\s\S]*const wifi = await directClient\.request\('get', 'wifi'\);/);
        ...
        assert.match(app, /await client\.request\('patch', 'sensing', \{ enabled: true \}\)/);
        ...
        assert.match(app, /if \(verification\) beginConfigVerification\(action, verification, result\?\.data \|\| result\)/);
        assert.match(app, /directClient\.request\('delete', 'wifi\/credentials', \{\}, \{ timeoutMs: 3000 \}\)/);
        ...
        assert.match(app, /supportsOta \? await directClient\.request\('get', 'ota'\) : null/);
        ...
        assert.match(app, /monitorPublishCommand\(otaCommandFields\('start_ota'\)/);
    ...
    it('keeps Direct HTTP diagnostics and presents Relay as unavailable', () => {
        ...
        assert.match(app, /command: 'read_diagnostics'/);
        ...
        assert.match(app, /function applyDirectConfig\(config\) \{[\s\S]*?const device = config\.device \|\| \{\};[\s\S]*?device_label: device\.label[\s\S]*?applySensingSnapshot\(runtime\);/);
        assert.match(app, /function applyRuntimeStatus\(status\) \{[\s\S]*?setCalibrationBusy\(calibrating\);/);
        assert.match(app, /if \(name === 'sensing'\)[\s\S]*?applyRuntimeStatus\(data\);/);
        ...
        assert.match(app, /function directConfigSnapshot\(config\) \{[\s\S]*?device_label: device\.label \?\? ''/);
    ...
    it('keeps MQTT configuration in Device settings while browser tools use HTTP', () => {
        ...
        assert.match(configurePage, /class="field-row mqtt-endpoint-row">[\s\S]*?id="cfg-mqtt-preset"[\s\S]*?id="cfg-mqtt-scheme"[\s\S]*?id="cfg-mqtt-host"[\s\S]*?id="cfg-mqtt-port"/);
        assert.match(configurePage, /id="cfg-mqtt-scheme"[\s\S]*?value="mqtt" selected[\s\S]*?value="mqtts"/);
        ...
        assert.match(app, /home_assistant:[\s\S]*?scheme: 'mqtt', host: 'homeassistant\.local', port: '1883'/);
        assert.match(app, /lan_broker:[\s\S]*?configure:[\s\S]*?scheme: 'mqtt', host: '', port: '1883'/);
        assert.match(app, /emqx_cloud:[\s\S]*?scheme: 'mqtts', host: 'deployment-id\.ala\.region\.emqxsl\.com', port: '8883'/);
        assert.match(app, /hivemq_cloud:[\s\S]*?scheme: 'mqtts', host: 'cluster-id\.s1\.region\.hivemq\.cloud', port: '8883'/);
        assert.match(app, /flespi:[\s\S]*?scheme: 'mqtts', host: 'mqtt\.flespi\.io', port: '8883'/);
        assert.match(app, /cloud_broker:[\s\S]*?configure:[\s\S]*?scheme: 'mqtts', host: 'cluster\.example\.com', port: ''/);
        ...
        assert.match(app, /locked: Object\.freeze\(\['scheme', 'port'\]\)/);
        assert.match(app, /locked: Object\.freeze\(\['scheme', 'host', 'port'\]\)/);
        assert.match(app, /input\.readOnly = isLocked/);
        assert.match(styles, /\.field :where\(input, select\)\[data-preset-locked\][\s\S]*?cursor: not-allowed/);
        assert.match(styles, /\.mqtt-endpoint-row \{ grid-template-columns: 152px 82px minmax\(180px, 1fr\) 72px; \}/);
        assert.doesNotMatch(styles, /\.broker-host-control/);
        assert.doesNotMatch(app, /'mqtts:\/\/' \+ enteredHost/);
    ...
    it('preserves write-only MQTT credentials and confirms destructive MQTT removal', () => {
        ...
        assert.match(app, /const mqttParams = \{ scheme, host, port, topic_prefix: topicPrefix \};[\s\S]*?if \(username \|\| clearCredentials\)[\s\S]*?if \(password \|\| clearCredentials\)/);
    ...
    it('reuses active Direct HTTP and Demo sessions for Raw CSI', () => {
        ...
        assert.match(rawClient, /const client = directClient;[\s\S]*fetch\(client\.rawEndpoint/);
        assert.doesNotMatch(rawClient, /makeDirectClient|client\.connect\(|client\.handshake\(|client\.close\(/);
        assert.match(rawClient, /new window\.ESPectreRawCsiParser\(\)/);
        assert.match(rawClient, /rawCsi\.parser\.append\(chunk\)\.forEach/);
        assert.match(rawClient, /state: 'idle'/);
        assert.match(rawClient, /track\('csi_stream'/);
        assert.match(rawClient, /markToolReady\('csi'\)/);
        assert.match(rawClient, /rawCsi\.analyticsSuccessTracked = track\('csi_stream'/);
        ...
        assert.doesNotMatch(rawClient, /startRequest|start_raw_stream|stop_raw_stream|Authorization/);
