...
void test_native_frontend_direct_reports_mqtt_connection_changes(void) {
  ...
  mqtt.emit_connection(true);
  TEST_ASSERT_TRUE(direct_http_service_mock::state.published_events.empty());
...
void test_native_frontend_live_telemetry_publishes_mqtt_telemetry(void) {
  ...
  TEST_ASSERT_TRUE(std::any_of(mqtt_transport_mock::state.publishes.begin(),
                               ...
                               [](const mqtt_transport_mock::Publish &publish) {
                                 return publish.topic == "espectre/v1/devices/0000abcdeffedcba/motion" &&
                                        publish.payload.find("\"state\":\"motion\"") != std::string::npos &&
                                        publish.payload.find("\"score\":2.75") != std::string::npos;
                               }));
}
...
void test_native_frontend_mqtt_set_threshold_command_publishes_result(void) {
  ...
  mqtt.emit_command("{\"command_id\":\"cmd-1\",\"command\":\"update_sensing\",\"threshold\":0.45}");
...
void test_native_frontend_mqtt_rejects_protocol_metadata(void) {
  MockMqttTransport mqtt;
  EspectreDeviceConfig config;
  config.device_id = 0x0000abcdeffedcbaULL;
  config.mqtt_scheme = "mqtt";
  ...
  mqtt.emit_command(
      "{\"command_id\":\"cmd-version\",\"command\":\"update_sensing\","
      "\"protocol_version\":\"2.0\",\"threshold\":0.4}");

  TEST_ASSERT_TRUE(!mqtt_transport_mock::state.publishes.empty());
  const auto &publish = mqtt_transport_mock::state.publishes.back();
  TEST_ASSERT_EQUAL_STRING("espectre/v1/devices/0000abcdeffedcba/commands/result", publish.topic.c_str());
  TEST_ASSERT_TRUE(publish.payload.find("\"command_id\":\"cmd-version\"") != std::string::npos);
  TEST_ASSERT_TRUE(publish.payload.find("\"command\":\"update_sensing\"") != std::string::npos);
  TEST_ASSERT_TRUE(publish.payload.find("\"accepted\":false") != std::string::npos);
  TEST_ASSERT_TRUE(publish.payload.find("\"code\":\"invalid_params\"") != std::string::npos);
}
...
void test_native_frontend_mqtt_set_device_label_persists_and_republishes_info(void) {
  ...
  mqtt.emit_command(
      "{\"command_id\":\"label-1\",\"command\":\"update_device\",\"label\":\"Kitchen Sensor\"}");
  ...
  TEST_ASSERT_TRUE(std::any_of(mqtt_transport_mock::state.publishes.begin(),
                               mqtt_transport_mock::state.publishes.end(),
                               [](const mqtt_transport_mock::Publish &publish) {
                                 return publish.topic == "espectre/v1/devices/0000abcdeffedcba/device" &&
                                        publish.retain &&
                                        publish.payload.find("\"label\":\"Kitchen Sensor\"") !=
                                            std::string::npos;
                               }));
  TEST_ASSERT_TRUE(!mqtt_transport_mock::state.publishes.empty());
  ...
  TEST_ASSERT_TRUE(result.payload.find("\"command\":\"update_device\"") != std::string::npos);
...
void test_native_frontend_mqtt_detector_command_updates_runtime(void) {
  ...
  mqtt_transport_mock::state.publishes.clear();
  mqtt.emit_command(
      "{\"command_id\":\"det-1\",\"command\":\"update_sensing\",\"detector\":\"lightweight\"}");
...
void test_native_frontend_mqtt_motion_hits_command_updates_runtime(void) {
  ...
  mqtt_transport_mock::state.publishes.clear();
  mqtt.emit_command(
      "{\"command_id\":\"motion-1\",\"command\":\"update_sensing\",\"motion_on_hits\":5,\"motion_off_hits\":3}");
...
void test_native_frontend_mqtt_traffic_commands_update_runtime(void) {
  ...
  mqtt_transport_mock::state.publishes.clear();
  mqtt.emit_command(
      "{\"command_id\":\"traffic-1\",\"command\":\"update_sensing\",\"traffic_generator_mode\":\"pacing\"}");
...
void test_native_frontend_mqtt_info_and_stats_commands_publish_protocol_payloads(void) {
  ...
  TEST_ASSERT_TRUE(mqtt_transport_mock::state.publishes[0].payload.find("\"uptime\":") != std::string::npos);
  TEST_ASSERT_TRUE(mqtt_transport_mock::state.publishes[0].payload.find("\"free_memory_kb\":") != std::string::npos);
  TEST_ASSERT_TRUE(mqtt_transport_mock::state.publishes[0].payload.find("\"loop_time_ms\":") != std::string::npos);
  TEST_ASSERT_TRUE(mqtt_transport_mock::state.publishes[0].payload.find("\"traffic_tx_pps\":100") !=
                   std::string::npos);
  TEST_ASSERT_TRUE(mqtt_transport_mock::state.publishes[0].payload.find("\"csi_callback_pps\":96") !=
                   std::string::npos);
...
void test_native_frontend_serializes_telemetry_once_for_active_transports(void) {
  ...
  TEST_ASSERT_EQUAL_STRING("motion", event.event_name.c_str());
  TEST_ASSERT_TRUE(event.replaceable_telemetry);
  TEST_ASSERT_TRUE(event.data_json.find("\"score\":2.5") != std::string::npos);
  const int mqtt_index = mqtt_publish_index("espectre/v1/devices/0000111122223333/motion");
...
int main(int argc, char **argv) {
  ...
  RUN_TEST(test_native_frontend_mqtt_rejects_protocol_metadata);
