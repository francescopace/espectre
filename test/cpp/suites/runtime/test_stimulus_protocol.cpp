...
std::array<uint8_t, STIMULUS_HEADER_BYTES> build_stimulus_payload_(uint32_t stimulus_id) {
  return {static_cast<uint8_t>('E'),
          ...
          static_cast<uint8_t>((stimulus_id >> 24U) & 0xFFU),
          static_cast<uint8_t>((stimulus_id >> 16U) & 0xFFU),
          static_cast<uint8_t>((stimulus_id >> 8U) & 0xFFU),
          static_cast<uint8_t>(stimulus_id & 0xFFU)};
}

std::array<uint8_t, 46> build_llc_snap_payload_(uint32_t src_ip_host_order,
                                                uint32_t dst_ip_host_order,
                                                uint32_t stimulus_id) {
  std::array<uint8_t, 46> payload{};
  payload[0] = 0xAAU;
  payload[1] = 0xAAU;
  payload[2] = 0x03U;
  payload[3] = 0x00U;
  ...
  auto stimulus = build_stimulus_payload_(stimulus_id);
  std::memcpy(payload.data() + 8U + 20U + 8U, stimulus.data(), stimulus.size());
  return payload;
}
...
void test_extract_stimulus_metadata_from_csi_accepts_llc_snap_for_local_ip(void) {
  StimulusMetadata metadata{};
  const uint32_t collector_ip = inet_addr("192.168.1.22");
  const uint32_t local_ip = inet_addr("192.168.1.17");
  const auto payload = build_llc_snap_payload_(0xC0A80116U, 0xC0A80111U, 123U);
  const wifi_csi_info_t info = build_csi_info_(payload.data(), payload.size(), kCollectorMac, kLocalMac);

  TEST_ASSERT_TRUE(extract_stimulus_metadata_from_csi(&info, collector_ip, local_ip, kLocalMac, &metadata));
  TEST_ASSERT_EQUAL(123U, metadata.stimulus_id);
  TEST_ASSERT_EQUAL(0xC0A80111U, metadata.dst_ipv4_addr);
}

void test_csi_frame_matches_local_identity_rejects_other_unicast_mac(void) {
  const auto payload = build_stimulus_payload_(122U);
...
void test_csi_frame_matches_local_identity_accepts_broadcast_mac(void) {
  ...
  const auto payload = build_stimulus_payload_(123U);
...
void test_extract_stimulus_metadata_from_csi_rejects_llc_snap_for_other_ip(void) {
  StimulusMetadata metadata{};
  const uint32_t collector_ip = inet_addr("192.168.1.22");
  const uint32_t local_ip = inet_addr("192.168.1.17");
  const auto payload = build_llc_snap_payload_(0xC0A80116U, 0xC0A80118U, 456U);
  const wifi_csi_info_t info = build_csi_info_(payload.data(), payload.size(), kCollectorMac, kOtherMac);

  TEST_ASSERT_FALSE(extract_stimulus_metadata_from_csi(&info, collector_ip, local_ip, kLocalMac, &metadata));
}

void test_extract_stimulus_metadata_from_csi_rejects_direct_estm_for_other_mac(void) {
  StimulusMetadata metadata{};
  const uint32_t collector_ip = inet_addr("192.168.1.22");
  const uint32_t local_ip = inet_addr("192.168.1.17");
  const auto payload = build_stimulus_payload_(789U);
  const wifi_csi_info_t info = build_csi_info_(payload.data(), payload.size(), kCollectorMac, kOtherMac);

  TEST_ASSERT_FALSE(extract_stimulus_metadata_from_csi(&info, collector_ip, local_ip, kLocalMac, &metadata));
}

void test_extract_stimulus_metadata_from_csi_accepts_direct_estm_for_broadcast_mac(void) {
  StimulusMetadata metadata{};
  const uint32_t collector_ip = inet_addr("192.168.1.22");
  const uint32_t local_ip = inet_addr("192.168.1.17");
  const uint8_t broadcast_mac[6] = {0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU};
  const auto payload = build_stimulus_payload_(790U);
  const wifi_csi_info_t info = build_csi_info_(payload.data(), payload.size(), kCollectorMac, broadcast_mac);

  TEST_ASSERT_TRUE(extract_stimulus_metadata_from_csi(&info, collector_ip, local_ip, kLocalMac, &metadata));
  TEST_ASSERT_EQUAL(790U, metadata.stimulus_id);
}
...
int main() {
  ...
  RUN_TEST(test_extract_stimulus_metadata_from_csi_accepts_llc_snap_for_local_ip);
  RUN_TEST(test_extract_stimulus_metadata_from_csi_rejects_llc_snap_for_other_ip);
  RUN_TEST(test_extract_stimulus_metadata_from_csi_rejects_direct_estm_for_other_mac);
  RUN_TEST(test_extract_stimulus_metadata_from_csi_accepts_direct_estm_for_broadcast_mac);
  return end_suite();
}
