...
Install the OpenSSL development library (`libssl-dev` on Debian/Ubuntu, or `openssl@3` with Homebrew). The identity suite uses real host SHA-256 through adapters for the mbedTLS and PSA APIs, with known vectors, a deterministic station MAC, initialization and hashing failures, and invalid digest lengths. These tests validate identity derivation and error handling; they do not execute ESP-IDF's hardware crypto drivers. Run both branches with `./test/cpp/run_all_tests.sh -R test_device_identity`.
