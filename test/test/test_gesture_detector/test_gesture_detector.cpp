...
void test_gesture_features_output_range(void) {
    ...
    TEST_ASSERT_TRUE(std::isfinite(features[9]));                    // turb_mid_mean finite
    TEST_ASSERT_TRUE(std::isfinite(features[10]));                   // turb_late_mean finite
    TEST_ASSERT_TRUE(std::isfinite(features[11]));                   // turb_late_minus_mid finite
    TEST_ASSERT_TRUE(features[12] >= 0.0f);                          // turb_diff_abs_mean >= 0
    TEST_ASSERT_TRUE(features[13] >= 0.0f);                          // phase_diff_var >= 0
    TEST_ASSERT_TRUE(features[14] >= 0.0f);                          // phase_entropy >= 0
    TEST_ASSERT_TRUE(features[15] >= 0.0f && features[15] <= 1.0f); // circular variance [0,1]
}
...
void test_cross_platform_phases_constant(void) {
    ...
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, 0.0f, features[13]);

    // phase_entropy = 0 (flat phase per packet)
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, 0.0f, features[14]);

    // circular variance = 0 (all phases identical)
    TEST_ASSERT_FLOAT_WITHIN(1e-6f, 0.0f, features[15]);
}
...
void test_cross_platform_phases_linear_ramp(void) {
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.02f, 0.5258f, features[15]);
...
void test_gesture_feature_turb_diff_abs_mean_known_value(void) {
    // Consecutive differences are all 1.0 -> mean absolute diff = 1.0
    constexpr uint16_t N = 5;
    float turb[N] = {0.0f, 1.0f, 2.0f, 3.0f, 4.0f};
    float phases[N * 12];
    memset(phases, 0, sizeof(phases));
...
void test_gesture_feature_late_minus_mid_negative(void) {
    // Middle third has high turbulence, late third low turbulence.
    // Expect turb_late_minus_mid (feature 11) to be negative.
    constexpr uint16_t N = 9;
    float turb[N] = {
        1.0f, 1.0f, 1.0f,   // early third
        10.0f, 10.0f, 10.0f, // middle third
        1.0f, 1.0f, 1.0f    // late third
    };
    float phases[N * 12];
    memset(phases, 0, sizeof(phases));
...
void test_gesture_feature_phase_entropy_uniform_bins(void) {
    // Five evenly-populated bins -> entropy = log2(5) ~= 2.321928
    constexpr uint16_t N = 10;
    constexpr uint8_t N_PH = 5;
    float turb[N];
    float phases[N * N_PH];

    for (uint16_t p = 0; p < N; p++) {
        turb[p] = 1.0f;
        phases[p * N_PH + 0] = 0.0f;
        phases[p * N_PH + 1] = 1.0f;
        phases[p * N_PH + 2] = 2.0f;
        phases[p * N_PH + 3] = 3.0f;
...
int main(int argc, char **argv) {
    ...
    RUN_TEST(test_gesture_feature_turb_diff_abs_mean_known_value);
    RUN_TEST(test_gesture_feature_late_minus_mid_negative);
    RUN_TEST(test_gesture_feature_phase_entropy_uniform_bins);
