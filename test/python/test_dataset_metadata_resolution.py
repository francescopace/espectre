...
Tests for dataset metadata resolution helpers used by training and cache identity.
...
def test_resolve_dataset_selection_require_pair_excludes_non_pair_labels(monkeypatch, tmp_path) -> None:
    ...
    selected = dataset_metadata.resolve_dataset_selection(
        "static_presence_s3_64sc_room_001",
        num_sc=64,
        require_pair=True,
    )
    ...
    try:
        dataset_metadata.resolve_dataset_selection(
            "empty_s3_64sc_room_001",
            num_sc=64,
            require_pair=True,
        )
