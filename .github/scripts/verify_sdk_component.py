...
def load_package(path: Path) -> tuple[dict, dict[str, bytes]]:
    ...
    if (manifest["version"] != inventory["version"]
            or manifest["repository_info"]["commit_sha"] != inventory["commit"]
            or example["dependencies"][COMPONENT_NAME]["version"] != inventory["version"]):
        raise ValueError("Component, example, and inventory identities differ")
...
def registry_files(registry: str, inventory: dict, wait_seconds: int,
...
def prepare(args: argparse.Namespace, inventory: dict, files: dict[str, bytes]) -> None:
    ...
    if args.registry_url:
        metadata = registry_files(args.registry_url, inventory, args.wait_seconds)
        example_url = next(item["url"] for item in metadata["examples"] if item["name"] == "basic")
        downloaded_example = archive_files(download(example_url))
...
def verify_install(project: Path, inventory: dict, target: str) -> None:
...
def build(args: argparse.Namespace, inventory: dict) -> None:
    ...
    if args.registry_url:
        verify_install(project, inventory, args.target)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("command", choices=("check", "extract", "prepare", "build", "verify-install"))
    ...
    if args.snapshot_channel:
        ...
        suffix = f"-snapshot.{args.snapshot_channel}.g{args.expected_commit}"
        if not inventory["version"].endswith(suffix):
            raise ValueError("Component version does not match the snapshot channel and commit")
    if args.command == "check":
        exists = False
        if args.registry_url:
            exists = registry_files(args.registry_url, inventory, args.wait_seconds, args.allow_missing) is not None
        ...
        verify_install(args.destination / "basic", inventory, args.target)
