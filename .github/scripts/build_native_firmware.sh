...
docker run --rm \
  ...
  bash -lc "
    ...
    if python -m esptool merge-bin -h >/dev/null 2>&1; then
      python -m esptool --chip ${NATIVE_TARGET} merge-bin --pad-to-size 4MB -o /work/${NATIVE_OUTPUT#${REPO_ROOT}/} @flash_args
    else
      python -m esptool --chip ${NATIVE_TARGET} merge_bin --fill-flash-size 4MB -o /work/${NATIVE_OUTPUT#${REPO_ROOT}/} @flash_args
    fi
  "
